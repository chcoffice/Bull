...
#else
#define tcp_socket eXosip.xtl_tcp.socket
#define my_addr eXosip.xtl_tcp.addr

#define tcp_firewall_ip eXosip.xtl_tcp.firewall_ip
#define tcp_firewall_port eXosip.xtl_tcp.firewall_port
#endif
...
static int _tcp_tl_send_sockinfo (struct _tcp_sockets *sockinfo, const char *msg, int msglen MARGS);

#ifdef SIP_INSTANCE_ONE

static struct _tcp_sockets tcp_socket_tab[EXOSIP_MAX_SOCKETS];
#else
#define tcp_socket_tab ((struct _tcp_sockets*)eXosip.xtl_tcp.pPri)
#endif

static int tcp_tl_init( MARGS2 )
{
	tcp_socket = 0;
	memset(&my_addr, 0, sizeof(struct sockaddr_storage));
...
static void _tcp_tl_close_sockinfo(struct _tcp_sockets *sockinfo MARGS)
{
...
static int tcp_tl_free( MARGS2)
{
	...
	for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
		if (tcp_socket_tab[pos].socket > 0) {
			_tcp_tl_close_sockinfo(&tcp_socket_tab[pos] MPARAMS);
		}
	}
...
static int tcp_tl_open( MARGS2)
{
	...
	res = eXosip_get_addrinfo(&addrinfo,
							  eXtl_tcp.proto_ifs,
							  eXtl_tcp.proto_port, eXtl_tcp.proto_num MPARAMS);
	if (res)
		return -1;
	...
	for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
		...
		len = sizeof(my_addr);
		res = getsockname(sock, (struct sockaddr *) &my_addr, &len);
		if (res != 0) {
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_ERROR, NULL,
						"Cannot get socket name (%s)\n", strerror(ex_errno)));
			memcpy(&my_addr, curinfo->ai_addr, curinfo->ai_addrlen);
		}
	...
	if (eXtl_tcp.proto_port == 0) {
		...
		if (eXtl_tcp.proto_family == AF_INET)
			...
			eXtl_tcp.proto_port =
				ntohs(((struct sockaddr_in6 *) &my_addr)->sin6_port);
		OSIP_TRACE(osip_trace
...
static int tcp_tl_set_fdset(fd_set * osip_fdset, fd_set * osip_wrset, int *fd_max MARGS)
{
	int pos;
	if (tcp_socket <= 0)
		return -1;
...
/* Like strstr, but works for haystack that may contain binary data and is
   not NUL-terminated. */
static char *buffer_find(const char *haystack, size_t haystack_len, const char *needle MARGS)
{
...
static int handle_messages(struct _tcp_sockets *sockinfo MARGS)
{
	int consumed = 0;
	char *buf = sockinfo->buf;
	size_t buflen = sockinfo->buflen;
	char *end_headers;
	while (buflen > 0 && (end_headers = buffer_find(buf, buflen, END_HEADERS_STR MPARAMS)) != NULL) {
		int clen, msglen;
		char *clen_header;
		...
		*end_headers = END_HEADERS_STR[0];
		...
		_eXosip_handle_incoming_message(buf, msglen, sockinfo->socket,
										sockinfo->remote_ip, sockinfo->remote_port MPARAMS);
		consumed += msglen;
		buflen -= msglen;
		buf += msglen;
	}
...
static int _tcp_tl_recv(struct _tcp_sockets *sockinfo MARGS)
{
	int r;
	if (!sockinfo->buf) {
	...
	if (r == 0) {
		...
		_tcp_tl_close_sockinfo(sockinfo MPARAMS);
		eXosip_mark_all_registrations_expired( MPARAMS2);
		...
		sockinfo->buflen += r;
		consumed = handle_messages(sockinfo MPARAMS);
		if (consumed == 0) {
			return OSIP_SUCCESS;
		} else {
...
static int tcp_tl_read_message(fd_set * osip_fdset, fd_set * osip_wrset MARGS)
{
	int pos = 0;
	...
	for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
		if (tcp_socket_tab[pos].socket > 0) {
			if (FD_ISSET(tcp_socket_tab[pos].socket, osip_wrset))
				_tcp_tl_send_sockinfo(&tcp_socket_tab[pos], NULL, 0 MPARAMS);
			if (FD_ISSET(tcp_socket_tab[pos].socket, osip_fdset))
				_tcp_tl_recv(&tcp_socket_tab[pos] MPARAMS);
		}
	}
...
static struct _tcp_sockets *_tcp_tl_find_sockinfo(int sock MARGS)
{
	int pos;
...
static int _tcp_tl_find_socket(char *host, int port MARGS)
{
	int pos;
...
static int _tcp_tl_is_connected(int sock MARGS)
{
	int res;
	struct timeval tv;
	fd_set wrset;
...
static int _tcp_tl_check_connected( MARGS2 )
{
	int pos;
	int res;
			...
			&& tcp_socket_tab[pos].ai_addrlen > 0) {
				...
				if (res < 0) {
					...
					if (status != EINPROGRESS && status != EALREADY) {
						...
						res = _tcp_tl_is_connected(tcp_socket_tab[pos].socket MPARAMS);
						if (res > 0) {
							OSIP_TRACE(osip_trace
								(__FILE__, __LINE__, OSIP_INFO2, NULL,
...
static int _tcp_tl_connect_socket(char *host, int port MARGS)
{
	int pos;
	int res;
	struct addrinfo *addrinfo = NULL;
	...
	res = eXosip_get_addrinfo(&addrinfo, host, port, IPPROTO_TCP MPARAMS);
	if (res)
		return -1;
	...
	for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
		...
		if (res == 0) {
			int i = _tcp_tl_find_socket(src6host, port MPARAMS);
			if (i >= 0) {
				eXosip_freeaddrinfo(addrinfo);
				return i;
			}
		...
		if (res < 0) {
			...
			if (ex_errno != EINPROGRESS) {
				...
				res = _tcp_tl_is_connected(sock MPARAMS);
				if (res > 0) {
					OSIP_TRACE(osip_trace
							   (__FILE__, __LINE__, OSIP_INFO2, NULL,
...
static int
_tcp_tl_send_sockinfo (struct _tcp_sockets *sockinfo, const char *msg, int msglen MARGS)
{
	int i;
	while (1) {
...
static int
_tcp_tl_send (int sock, const char *msg, int msglen MARGS)
{
	struct _tcp_sockets *sockinfo = _tcp_tl_find_sockinfo (sock MPARAMS);
	if (sockinfo == NULL) {
		OSIP_TRACE (osip_trace (__FILE__, __LINE__, OSIP_INFO1, NULL,
								"could not find sockinfo for socket %d! dropping message\n", sock));
		return -1;
	}
	return _tcp_tl_send_sockinfo (sockinfo, msg, msglen MPARAMS);
}

static int
tcp_tl_send_message(osip_transaction_t * tr, osip_message_t * sip, char *host,
					int port, int out_socket MARGS)
{
	size_t length = 0;
	char *message = NULL;
	int i;
	...
	if (tr==NULL)
	{
		_eXosip_srv_lookup(sip, &naptr_record MPARAMS);

		if (naptr_record!=NULL) {
			eXosip_dnsutils_dns_process(naptr_record, 1 MPARAMS);
	...
	if (naptr_record!=NULL)
	{
		...
		eXosip_dnsutils_dns_process(naptr_record, 0 MPARAMS);
		if (naptr_record->naptr_state==OSIP_NAPTR_STATE_NAPTRDONE
			||naptr_record->naptr_state==OSIP_NAPTR_STATE_SRVINPROGRESS)
			eXosip_dnsutils_dns_process(naptr_record, 0 MPARAMS);
	...
	_tcp_tl_check_connected( MPARAMS2);
	...
	if (out_socket <= 0) {
		pos = _tcp_tl_find_socket(host, port MPARAMS);
		if (pos>=0)
		{
			OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
		...
		if (pos < 0) {
			pos = _tcp_tl_connect_socket(host, port MPARAMS);
		}
		if (pos>=0)
			out_socket = tcp_socket_tab[pos].socket;
	}
	...
	i = _tcp_tl_is_connected(out_socket MPARAMS);
	if (i > 0) {
		time_t now;
		now = time(NULL);
		OSIP_TRACE(osip_trace
		...
		if (tr != NULL && now - tr->birth_time > 10 && now - tr->birth_time < 13)
		{
			...
			{
				if (eXosip_dnsutils_rotate_srv(&naptr_record->siptcp_record MPARAMS)>0)
				{
					OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
	...
	OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL,
						  ...
						  host, port, message));
	i = _tcp_tl_send(out_socket, (const void *)message, length MPARAMS);
	osip_free(message);
	return i;
}

#ifdef ENABLE_KEEP_ALIVE_OPTIONS_METHOD
static int _tcp_tl_get_socket_info(int socket, char *host, int hostsize, int *port MARGS)
{
...
static int tcp_tl_keepalive( MARGS2)
{
	char buf[5] = "\r\n\r\n";
	int pos;
	int i;
	...
	for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
		if (tcp_socket_tab[pos].socket > 0) {
			i = _tcp_tl_is_connected(tcp_socket_tab[pos].socket MPARAMS);
			if (i > 0) {
				OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_INFO2, NULL,
...
static int tcp_tl_set_socket(int socket MARGS)
{
	tcp_socket = socket;
...
static int tcp_tl_masquerade_contact(const char *public_address, int port MARGS)
{
...
static int
tcp_tl_get_masquerade_contact(char *ip, int ip_size, char *port, int port_size MARGS)
{
	memset(ip, 0, ip_size);
...
static struct eXtl_protocol DefaultXtl_tcp = {
	1,
	5060,
	"TCP",
	"0.0.0.0",
...
{
#ifdef SIP_INSTANCE_ONE
	memcpy(eXtl, &eXtl_udp,sizeof(*eXtl));
#else
	memcpy(eXtl, &DefaultXtl_tcp,sizeof(*eXtl));
	DefaultXtl_tcp.proto_port++;
	eXtl->pPri = malloc(sizeof(struct _tcp_sockets)*(1+EXOSIP_MAX_SOCKETS));
#endif
}
