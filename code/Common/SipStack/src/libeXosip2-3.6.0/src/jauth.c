...
void DigestCalcResponseAka(IN const char *pszPassword, IN const char *pszNonce,	/* nonce from server */
						   IN const char *pszCNonce,	/* client nonce */
						   IN const char *pszQop,	/* qop-value: "", "auth", "auth-int" */
						   IN const char *pszMethod,	/* method from the request */
						   IN const char *pszDigestUri,	/* requested URL */
...
__eXosip_create_authorization_header(osip_www_authenticate_t * wa,
									 ...
									 const char *method,
									 const char *pCNonce, int iNonceCount MARGS)
{
...
// 			printf( "*********User:'%s',Realm:'%s',Pwd:'%s'\r\n"
// 					"*********Nonce:'%s', CNonce:'%s' Res:'%s'\r\n"
// 					"*********Qop:'%s',Method:'%s',NonceCount:'%s'\r\n"
// 					"*********Uri:'%s', Ver:'%d', HA1:'%s'\r\n",
// 				pszUser, pszRealm, pszPass, pszNonce, pszCNonce, Response,
// 				pszQop, pszMethod, szNonceCount, pszURI, version, (char*)pha1);
		} else {
