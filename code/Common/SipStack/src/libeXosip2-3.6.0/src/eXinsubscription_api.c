...
int eXosip_insubscription_remove(int did MARGS)
{
	...
	if (did > 0) {
		eXosip_notify_dialog_find(did, &jn, &jd MPARAMS);
	}
	if (jd == NULL || jn == NULL) {
...
int
eXosip_insubscription_build_answer(int tid, int status, osip_message_t ** answer MARGS)
{
	int i = -1;
	eXosip_dialog_t *jd = NULL;
	eXosip_notify_t *jn = NULL;
	...
	*answer = NULL;
	...
	if (tid > 0) {
		_eXosip_insubscription_transaction_find(tid, &jn, &jd, &tr MPARAMS);
	}
	...
	if (status >= 200 && status <= 299)
		_eXosip_notify_add_expires_in_2XX_for_subscribe(jn, *answer MPARAMS);
...
int eXosip_insubscription_send_answer(int tid, int status, osip_message_t * answer MARGS)
{
	int i = -1;
	eXosip_dialog_t *jd = NULL;
	eXosip_notify_t *jn = NULL;
	...
	if (answer == NULL) {
		if (0 == osip_strcasecmp(tr->orig_request->sip_method, "SUBSCRIBE")) {
			if (status < 200)
				i = _eXosip_insubscription_answer_1xx(jn, jd, status MPARAMS);
			else
				i = _eXosip_insubscription_answer_3456xx(jn, jd, status MPARAMS);
			if (i != 0) {
				OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_ERROR, NULL,
								   ...
								   osip_message_t ** request MARGS)
{
	...
	*request = NULL;
	...
	i = eXosip_insubscription_build_request(did, "NOTIFY", request MPARAMS);
	if (i != 0) {
		return i;
	}
#ifndef SUPPORT_MSN
									...
									osip_message_t ** request MARGS)
{
	...
	*request = NULL;
	...
	transaction = NULL;
	transaction = eXosip_find_last_out_notify(jn, jd MPARAMS);
	if (transaction != NULL) {
	...
	if (transaction != NULL && transaction->orig_request != NULL)
		transport = _eXosip_transport_protocol(transaction->orig_request MPARAMS);
...
int eXosip_insubscription_send_request(int did, osip_message_t * request MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_notify_t *jn = NULL;
													...
													osip_transaction_t * out_tr MARGS)
{
	...
	if (out_tr == NULL) {
		out_tr = eXosip_find_last_out_notify(jn, jd MPARAMS);
	}
	...
	osip_transaction_set_your_instance(tr, __eXosip_new_jinfo(NULL, jd, NULL, jn MPARAMS));
										...
										int subscription_reason MARGS)
{
	...
	i = eXosip_insubscription_build_notify(did, subscription_status,
										   subscription_reason, &notify MPARAMS);
	if (i != 0) {
		return i;
	}
	...
	return eXosip_insubscription_send_request(did, notify MPARAMS);
}

int eXosip_insubscription_automatic(eXosip_event_t * evt MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_notify_t *jn = NULL;
	...
	eXosip_notify_dialog_find(evt->did, &jn, &jd MPARAMS);
	if (jd == NULL || jn == NULL) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					"eXosip: No incoming subscription here?\n"));
	...
	if (event_header == NULL || event_header->hvalue == NULL) {
		eXosip_insubscription_send_answer(evt->tid, 400, NULL MPARAMS);
		return OSIP_SUCCESS;
	}
	...
	if (osip_strcasecmp(event_header->hvalue, "dialog") == 0) {
		...
		if (evt->type == EXOSIP_IN_SUBSCRIPTION_NEW) {
			...
			i = eXosip_insubscription_build_answer(evt->tid, 202, &answer MPARAMS);
			if (i == 0) {
				i = eXosip_insubscription_send_answer(evt->tid, 202, answer MPARAMS);
			}
			if (i != 0) {
				i = eXosip_insubscription_send_answer(evt->tid, 400, NULL MPARAMS);
				return OSIP_SUCCESS;
			}
		...
		if (evt->type == EXOSIP_IN_SUBSCRIPTION_NEW) {
			eXosip_insubscription_send_answer(evt->tid, 489, NULL MPARAMS);
		}
	}
