...
static int cancel_match_invite(osip_transaction_t * invite,
							   osip_message_t * cancel MARGS);
static void eXosip_process_cancel(osip_transaction_t * transaction,
								  osip_event_t * evt MARGS);
static void eXosip_process_reinvite(eXosip_call_t * jc,
									eXosip_dialog_t * jd,
									osip_transaction_t *
									transaction, osip_event_t * evt MARGS);
static void eXosip_process_new_invite(osip_transaction_t * transaction,
									  osip_event_t * evt MARGS);
static void eXosip_process_new_register(osip_transaction_t * transaction,
									  osip_event_t * evt MARGS);


static void eXosip_process_new_message(osip_transaction_t * transaction,
									   osip_event_t * evt MARGS);
...
static int eXosip_match_notify_for_subscribe(eXosip_subscribe_t * js,
											 osip_message_t * notify MARGS);
#endif
static void eXosip_process_message_within_dialog(eXosip_call_t * jc,
												 eXosip_dialog_t * jd,
												 osip_transaction_t *
												 transaction, osip_event_t * evt MARGS);
static void eXosip_process_newrequest(osip_event_t * evt, int socket MARGS);
static void eXosip_process_response_out_of_transaction(osip_event_t * evt MARGS);
static int eXosip_pendingosip_transaction_exist(eXosip_call_t * jc,
												eXosip_dialog_t * jd MARGS);
static int eXosip_release_finished_calls(eXosip_call_t * jc, eXosip_dialog_t * jd MARGS);
static int eXosip_release_aborted_calls(eXosip_call_t * jc, eXosip_dialog_t * jd MARGS);
...
eXosip_send_default_answer(eXosip_dialog_t * jd,
						   ...
						   osip_event_t * evt,
						   int status,
						   char *reason_phrase, char *warning, int line MARGS)
{
	...
	if (jd != NULL)
		...
		i = _eXosip_build_response_default(&answer, NULL, status, evt->sip MPARAMS);
	...
	__eXosip_wakeup( MPARAMS2);

}

static void
eXosip_process_bye(eXosip_call_t * jc, eXosip_dialog_t * jd,
				   osip_transaction_t * transaction, osip_event_t * evt MARGS)
{
	osip_event_t *evt_answer;
	osip_message_t *answer;
	int i;
...
#else
	osip_transaction_set_your_instance(transaction,
									   __eXosip_new_jinfo(jc, NULL /*jd */  MPARAMS));
#endif

	i = _eXosip_build_response_default(&answer, jd->d_dialog, 200, evt->sip MPARAMS);
	if (i != 0) {
		osip_list_add(&eXosip.j_transactions, transaction, 0);
		return;
	}
	...
	report_call_event(EXOSIP_CALL_MESSAGE_NEW, jc, jd, transaction MPARAMS);
	report_call_event(EXOSIP_CALL_CLOSED, jc, jd, transaction MPARAMS);
	eXosip_update( MPARAMS2);			/* AMD 30/09/05 */

	__eXosip_wakeup( MPARAMS2);
}

static void
eXosip_process_ack(eXosip_call_t * jc, eXosip_dialog_t * jd, osip_event_t * evt MARGS)
{
	...
	je = eXosip_event_init_for_call(EXOSIP_CALL_ACK, jc, jd, NULL MPARAMS);
	if (je != NULL) {
		osip_transaction_t *tr;
		tr = eXosip_find_last_inc_invite(jc, jd MPARAMS);
		if (tr != NULL) {
...
#ifndef MINISIZE
	osip_transaction_set_your_instance(transaction,
									   __eXosip_new_jinfo(jc, jd, NULL, NULL MPARAMS));
#else
	osip_transaction_set_your_instance(transaction, __eXosip_new_jinfo(jc, jd MPARAMS));
#endif
...
static int
cancel_match_invite(osip_transaction_t * invite, osip_message_t * cancel MARGS)
{
...
static void
eXosip_process_cancel(osip_transaction_t * transaction, osip_event_t * evt MARGS)
{
	osip_transaction_t *tr;
	osip_event_t *evt_answer;
	osip_message_t *answer;
	...
	for (jc = eXosip.j_calls; jc != NULL; jc = jc->next) {
		if (jc->c_inc_tr != NULL) {
			i = cancel_match_invite(jc->c_inc_tr, evt->sip MPARAMS);
			if (i == 0) {
				tr = jc->c_inc_tr;
				/* fixed */
		...
		for (jd = jc->c_dialogs; jd != NULL; jd = jd->next) {
			...
			while (!osip_list_eol(jd->d_inc_trs, pos)) {
				...
				i = cancel_match_invite(tr, evt->sip MPARAMS);
				if (i == 0)
					break;
				tr = NULL;
				pos++;
	...
	if (tr == NULL) {			/* we didn't found the transaction to cancel */
		i = _eXosip_build_response_default(&answer, NULL, 481, evt->sip MPARAMS);
		if (i != 0) {
	...
	{
		if (jd == NULL)
			i = _eXosip_build_response_default(&answer, NULL, 200, evt->sip MPARAMS);
		else
						...
						osip_transaction_t * transaction, osip_event_t * evt MARGS)
{
	...
	report_call_event(EXOSIP_CALL_REINVITE, jc, jd, transaction MPARAMS);
}
										...
										osip_event_t * evt MARGS)
{
	...
	osip_message_t *answer;
	int i;
	eXosip_call_init(&jc MPARAMS);

	i = _eXosip_build_response_default(&answer, NULL, 101, evt->sip MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
			(__FILE__, __LINE__, OSIP_ERROR, NULL,
			"eXosip: cannot create dialog."));
		eXosip_call_free(jc MPARAMS);
		return;
	}
	osip_message_set_content_length(answer, "0");
	i = complete_answer_that_establish_a_dialog(answer, evt->sip MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
			(__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	i = eXosip_dialog_init_as_uas(&jd, evt->sip, answer MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
			(__FILE__, __LINE__, OSIP_ERROR, NULL,
			"eXosip: cannot create dialog!\n"));		
...
// #endif
	...
	__eXosip_wakeup( MPARAMS2);			/* needed? */
}
...
static void
eXosip_process_new_invite(osip_transaction_t * transaction, osip_event_t * evt MARGS)
{
	osip_event_t *evt_answer;
	int i;
	eXosip_call_t *jc;
	...
	if (transaction->orig_request != NULL) {
		report_call_event(EXOSIP_CALL_INVITE, jc, jd, transaction MPARAMS);
	}
...
static void
eXosip_process_new_subscribe(osip_transaction_t * transaction, osip_event_t * evt MARGS)
{
	osip_event_t *evt_answer;
	eXosip_notify_t *jn;
	eXosip_dialog_t *jd;
	...
	i = eXosip_notify_init(&jn, evt->sip MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_ERROR, NULL,
							  "ERROR: missing contact or memory\n"));
		i = _eXosip_build_response_default(&answer, NULL, 400, evt->sip MPARAMS);
		if (i == 0) {
			evt_answer = osip_new_outgoing_sipmessage(answer);
			evt_answer->transactionid = transaction->transactionid;
			eXosip_update( MPARAMS2);
	...
	_eXosip_notify_set_refresh_interval(jn, evt->sip MPARAMS);
	...
	osip_transaction_set_your_instance(transaction,
									   __eXosip_new_jinfo(NULL, jd, NULL, jn MPARAMS));
									...
									osip_event_t * evt MARGS)
{
	...
	if (jd == NULL) {
		...
		eXosip_send_default_answer(jd, transaction, evt, 500,
								   "Internal SIP Error",
								   "No dialog for this NOTIFY", __LINE__ MPARAMS);
		return;
	}
	...
	if (i != 0) {
		...
		eXosip_send_default_answer(jd, transaction, evt, 500,
								   "Internal SIP Error",
								   "Failed to build Answer for NOTIFY", __LINE__ MPARAMS);
		return;
	}
#ifdef SUPPORT_MSN
	...
	if (0 == osip_strncasecmp(sub_state->hvalue, "terminated", 10)) {
		...
		REMOVE_ELEMENT(eXosip.j_subscribes, js);
		eXosip_subscribe_free(js MPARAMS);
		__eXosip_wakeup( MPARAMS2);
		return;
	} else {
		osip_transaction_set_your_instance(transaction,
										   __eXosip_new_jinfo(NULL, jd, js, NULL MPARAMS));
	}
#endif
...
static int
eXosip_match_notify_for_subscribe(eXosip_subscribe_t * js, osip_message_t * notify MARGS)
{
	...
	out_sub = eXosip_find_last_out_subscribe(js, NULL MPARAMS);
...
static void eXosip_process_newrequest(osip_event_t * evt, int socket MARGS)
{
	...
	if (MSG_IS_CANCEL(evt->sip)) {
		...
		eXosip_process_cancel(transaction, evt MPARAMS);
		return;
	}
	...
	if (jd != NULL && transaction != NULL && evt->sip != NULL
		...
		if (jd->d_dialog != NULL && jd->d_dialog->remote_cseq > 0) {
			...
			if (cseq == jd->d_dialog->remote_cseq) {
				...
				_eXosip_dnsutils_release(transaction->naptr_record MPARAMS);
			...
			if (cseq <= jd->d_dialog->remote_cseq) {
				eXosip_send_default_answer(jd, transaction, evt, 500,
										   NULL, "Wrong Lower CSeq", __LINE__ MPARAMS);
				return;
			}
		}
	}
#ifndef MINISIZE
	if (0 && ctx_type == IST) { //zouyx disable
		i = _eXosip_build_response_default(&answer, NULL, 100, evt->sip MPARAMS);
		if (i != 0) {
			__eXosip_delete_jinfo(transaction MPARAMS);
			_eXosip_dnsutils_release(transaction->naptr_record MPARAMS);
			transaction->naptr_record=NULL;
			osip_transaction_free(transaction);
			return;
	...
	if (jd != NULL) {
		...
		if (transaction == NULL) {
			...
			old_trn = eXosip_find_last_inc_transaction(jc, jd, "BYE" MPARAMS);
			if (old_trn == NULL)
				old_trn = eXosip_find_last_out_transaction(jc, jd, "BYE" MPARAMS);

			if (old_trn != NULL) {
				osip_list_add(&eXosip.j_transactions, transaction, 0);
				eXosip_send_default_answer(jd, transaction, evt, 481, NULL,
										   NULL, __LINE__ MPARAMS);
				return;
			}
		}
		...
		if (MSG_IS_INVITE(evt->sip)) {
			...
			old_trn = eXosip_find_last_inc_invite(jc, jd MPARAMS);
			...
			old_trn = eXosip_find_last_out_invite(jc, jd MPARAMS);
			...
			eXosip_process_reinvite(jc, jd, transaction, evt MPARAMS);
		} else if (MSG_IS_BYE(evt->sip)) {
			...
			if (tag_to == NULL || tag_to->gvalue == NULL) {
				...
				eXosip_send_default_answer(jd, transaction, evt, 481,
										   "Missing tags in BYE",
										   "Missing tags in BYE", __LINE__ MPARAMS);
				return;
			}
			...
			eXosip_process_bye(jc, jd, transaction, evt MPARAMS);
		} else if (MSG_IS_ACK(evt->sip)) {
			eXosip_process_ack(jc, jd, evt MPARAMS);
		} else {
			eXosip_process_message_within_dialog(jc, jd, transaction, evt MPARAMS);
		}
		return;
	}
	...
	if (MSG_IS_INVITE(evt->sip)) {
		eXosip_process_new_invite(transaction, evt MPARAMS);
		return;
	} else if (MSG_IS_REGISTER(evt->sip)) {
			eXosip_process_new_register(transaction, evt MPARAMS);
			return;
	} else if (MSG_IS_BYE(evt->sip)) {
		osip_list_add(&eXosip.j_transactions, transaction, 0);
		eXosip_send_default_answer(jd, transaction, evt, 481, NULL, NULL,
	...
	if (js != NULL) {
		...
		if (MSG_IS_MESSAGE(evt->sip)) {
			...
			/* the previous transaction MUST be freed */
			old_trn = eXosip_find_last_inc_notify(js, jd MPARAMS);
			...
			eXosip_process_notify_within_dialog(js, jd, transaction, evt MPARAMS);
		} else {
			osip_list_add(&eXosip.j_transactions, transaction, 0);
			eXosip_send_default_answer(jd, transaction, evt, 501, NULL,
									   "Just Not Implemented", __LINE__ MPARAMS);
		}
		return;
	}
	...
	if (MSG_IS_NOTIFY(evt->sip)) {
		...
		/* first, look for a Dialog in the map of element */
		for (js = eXosip.j_subscribes; js != NULL; js = js->next) {
			if (eXosip_match_notify_for_subscribe(js, evt->sip MPARAMS) == 0) {
				i = eXosip_dialog_init_as_uac(&jd, evt->sip MPARAMS);
				if (i != 0) {
					OSIP_TRACE(osip_trace
							   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	if (jn != NULL) {
		...
		if (MSG_IS_MESSAGE(evt->sip)) {
			...
			/* the previous transaction MUST be freed */
			old_trn = eXosip_find_last_inc_subscribe(jn, jd MPARAMS);
			...
			eXosip_process_subscribe_within_call(jn, jd, transaction, evt MPARAMS);
		} else {
			osip_list_add(&eXosip.j_transactions, transaction, 0);
	...
	if( MSG_IS_MESSAGE(evt->sip) )
	{
		eXosip_process_new_message(transaction, evt MPARAMS);
		return;
	}

#ifndef MINISIZE
	if (MSG_IS_SUBSCRIBE(evt->sip)) {
		eXosip_process_new_subscribe(transaction, evt MPARAMS);
		return;
	}
#endif
...
static void eXosip_process_response_out_of_transaction(osip_event_t * evt MARGS)
{
	...
	if (jc != NULL && jd != NULL) {
		...
		if (jd->d_ack != NULL && jd->d_ack->cseq != NULL
			...
			if (0 ==
				...
				cb_snd_message(NULL, jd->d_ack, NULL, 0, -1 MPARAMS);
				OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_INFO1, NULL,
	...
	if (jc != NULL) {
...

		{
			...
			char *transport = _eXosip_transport_protocol(evt->sip MPARAMS);
#ifndef MINISIZE				/* Don't send ACK in MINISIZE mode to save code size */
			osip_message_t *ack;
			if (transport == NULL)
				i = _eXosip_build_request_within_dialog(&ack, "ACK", dlg, "UDP" MPARAMS);
			else
			...
			cb_snd_message(NULL, ack, NULL, 0, -1 MPARAMS);
			...
			/* ready to send a BYE */
			if (transport == NULL)
				i = generating_bye(&bye, dlg, "UDP" MPARAMS);
			else
				i = generating_bye(&bye, dlg, transport MPARAMS);
			if (bye != NULL && i == OSIP_SUCCESS)
				cb_snd_message(NULL, bye, NULL, 0, -1 MPARAMS);
								...
								char *host, int port MARGS)
{
	...
	if (i != 0) {
		...
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_INFO1, NULL,
					"This is a request\n", buf));
		eXosip_lock( MPARAMS2);
		if (MSG_IS_REQUEST(se->sip))
			eXosip_process_newrequest(se, socket MPARAMS);
		else if (MSG_IS_RESPONSE(se->sip))
			eXosip_process_response_out_of_transaction(se MPARAMS);
		eXosip_unlock( MPARAMS2);
	} else {
		/* handled by oSIP ! */
		return OSIP_SUCCESS;
	}
...
int eXosip_read_message(int max_message_nb, int sec_max, int usec_max MARGS)
{
	fd_set osip_fdset;
	fd_set osip_wrset;
	struct timeval tv;
	...
	while (max_message_nb != 0 && eXosip.j_stop_ua == 0) {
		...
		FD_ZERO(&osip_fdset);
		FD_ZERO(&osip_wrset);
		eXtl_udp.tl_set_fdset(&osip_fdset, &osip_wrset, &max MPARAMS);
		eXtl_tcp.tl_set_fdset(&osip_fdset, &osip_wrset, &max MPARAMS);
#ifdef HAVE_OPENSSL_SSL_H
#if !(OPENSSL_VERSION_NUMBER < 0x00908000L)
		eXtl_dtls.tl_set_fdset(&osip_fdset, &osip_wrset, &max MPARAMS);
#endif
		eXtl_tls.tl_set_fdset(&osip_fdset, &osip_wrset, &max MPARAMS);
#endif
		...
		if (0 == i || eXosip.j_stop_ua != 0) {
			...
			eXtl_udp.tl_read_message(&osip_fdset, &osip_wrset MPARAMS);
			eXtl_tcp.tl_read_message(&osip_fdset, &osip_wrset MPARAMS);
#ifdef HAVE_OPENSSL_SSL_H
#if !(OPENSSL_VERSION_NUMBER < 0x00908000L)
			eXtl_dtls.tl_read_message(&osip_fdset, &osip_wrset MPARAMS);
#endif
			eXtl_tls.tl_read_message(&osip_fdset, &osip_wrset MPARAMS);
#endif
		}
...
static int
eXosip_pendingosip_transaction_exist(eXosip_call_t * jc, eXosip_dialog_t * jd MARGS)
{
	osip_transaction_t *tr;
	time_t now = time(NULL);

	tr = eXosip_find_last_inc_transaction(jc, jd, "BYE" MPARAMS);
	if (tr != NULL && tr->state != NIST_TERMINATED) {	/* Don't want to wait forever on broken transaction!! */
		if (tr->birth_time + 180 < now) {	/* Wait a max of 2 minutes */
			...
			eXosip_remove_transaction_from_call(tr, jc MPARAMS);
			osip_list_add(&eXosip.j_transactions, tr, 0);
		} else
			return OSIP_SUCCESS;
	}

	tr = eXosip_find_last_out_transaction(jc, jd, "BYE" MPARAMS);
	...
	tr = eXosip_find_last_out_invite(jc, jd MPARAMS);
	...
	tr = eXosip_find_last_inc_transaction(jc, jd, "REFER" MPARAMS);
	if (tr != NULL && tr->state != NIST_TERMINATED) {	/* Don't want to wait forever on broken transaction!! */
		if (tr->birth_time + 180 < now) {	/* Wait a max of 2 minutes */
			/* remove the transaction from oSIP: */
			osip_remove_transaction(eXosip.j_osip, tr);
	...
	tr = eXosip_find_last_out_transaction(jc, jd, "REFER" MPARAMS);
	if (tr != NULL && tr->state != NICT_TERMINATED) {	/* Don't want to wait forever on broken transaction!! */
		if (tr->birth_time + 180 < now) {	/* Wait a max of 2 minutes */
			/* remove the transaction from oSIP: */
			osip_remove_transaction(eXosip.j_osip, tr);
...
static int
eXosip_release_finished_transactions(eXosip_call_t * jc, eXosip_dialog_t * jd MARGS)
{
	time_t now = time(NULL);
	...
	last_invite = eXosip_find_last_inc_invite(jc, jd MPARAMS);
	...
	if (jd != NULL) {
		...
		last_invite = eXosip_find_last_out_invite(jc, jd MPARAMS);
...
static int eXosip_release_finished_calls(eXosip_call_t * jc, eXosip_dialog_t * jd MARGS)
{
	...
	if (tr != NULL
		...
		__eXosip_call_remove_dialog_reference_in_call(jc, jd MPARAMS);
		REMOVE_ELEMENT(jc->c_dialogs, jd);
		eXosip_dialog_free(jd MPARAMS);
		return OSIP_SUCCESS;
	}
	return OSIP_UNDEFINED_ERROR;
}
...
static void __eXosip_release_call(eXosip_call_t * jc, eXosip_dialog_t * jd MARGS)
{
	REMOVE_ELEMENT(eXosip.j_calls, jc);
	report_call_event(EXOSIP_CALL_RELEASED, jc, jd, NULL MPARAMS);
	eXosip_call_free(jc MPARAMS);
	__eXosip_wakeup( MPARAMS2);
}
...
static int eXosip_release_aborted_calls(eXosip_call_t * jc, eXosip_dialog_t * jd MARGS)
{
	...
	if (tr != NULL && tr->state != IST_TERMINATED && tr->state != ICT_TERMINATED && tr->birth_time + 180 < now) {	/* Wait a max of 2 minutes */
		if (jd != NULL) {
			...
			__eXosip_call_remove_dialog_reference_in_call(jc, jd MPARAMS);
			REMOVE_ELEMENT(jc->c_dialogs, jd);
			report_call_event(EXOSIP_CALL_NOANSWER, jc, jd, NULL MPARAMS);
			eXosip_dialog_free(jd MPARAMS);
			__eXosip_wakeup( MPARAMS2);
			return OSIP_SUCCESS;
		}
	...
	if (tr != NULL && (tr->state == IST_TERMINATED || tr->state == ICT_TERMINATED)) {
		if (tr == jc->c_inc_tr) {
			if (jc->c_inc_tr->last_response == NULL) {
				...
				__eXosip_release_call(jc, jd MPARAMS);
				return OSIP_SUCCESS;
...
void eXosip_release_terminated_calls( MARGS2)
{
	eXosip_dialog_t *jd;
	...
	for (jc = eXosip.j_calls; jc != NULL;) {
		...
		for (jd = jc->c_dialogs; jd != NULL;) {
			jdnext = jd->next;
			if (0 == eXosip_pendingosip_transaction_exist(jc, jd MPARAMS)) {
			} else if (0 == eXosip_release_finished_transactions(jc, jd MPARAMS)) {
			} else if (0 == eXosip_release_finished_calls(jc, jd MPARAMS)) {
				jd = jc->c_dialogs;
			} else if (0 == eXosip_release_aborted_calls(jc, jd MPARAMS)) {
				jdnext = NULL;
			} else if (jd->d_id == -1) {
				OSIP_TRACE(osip_trace(__FILE__, __LINE__, OSIP_INFO2, NULL,
	...
	for (jc = eXosip.j_calls; jc != NULL;) {
		...
		if (jc->c_dialogs == NULL) {
			if (jc->c_inc_tr != NULL
				...
				__eXosip_release_call(jc, NULL MPARAMS);
	...
	while (!osip_list_eol(&eXosip.j_transactions, pos)) {
		...
		if (tr->state == NICT_TERMINATED
			...
			__eXosip_delete_jinfo(tr MPARAMS);
			_eXosip_dnsutils_release(tr->naptr_record MPARAMS);
...
void eXosip_release_terminated_registrations( MARGS2)
{
	eXosip_reg_t *jr;
	eXosip_reg_t *jrnext;
	time_t now = time(NULL);
...
void eXosip_release_terminated_publications( MARGS2)
{
	eXosip_pub_t *jpub;
...
static int
eXosip_release_finished_transactions_for_subscription(eXosip_dialog_t * jd MARGS)
{
	time_t now = time(NULL);
	osip_transaction_t *inc_tr;
	osip_transaction_t *out_tr;
...
void eXosip_release_terminated_subscriptions( MARGS2)
{
	time_t now = time(NULL);
	eXosip_dialog_t *jd;
	eXosip_dialog_t *jdnext;
	...
	for (js = eXosip.j_subscribes; js != NULL;) {
		...
		if (js->s_dialogs == NULL) {
			...
			jd = js->s_dialogs;
			if (jd!= NULL) {
				osip_transaction_t *transaction = eXosip_find_last_out_subscribe(js, jd MPARAMS);
				if (transaction != NULL
			...
			for (jd = js->s_dialogs; jd != NULL;) {
				jdnext = jd->next;
				eXosip_release_finished_transactions_for_subscription(jd MPARAMS);
...
void eXosip_release_terminated_in_subscriptions( MARGS2)
{
	eXosip_dialog_t *jd;
	eXosip_dialog_t *jdnext;
	eXosip_notify_t *jn;
