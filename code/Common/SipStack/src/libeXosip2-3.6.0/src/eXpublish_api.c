...
eXosip_build_publish(osip_message_t ** message,
					 ...
					 const char *route,
					 const char *event,
					 const char *expires, const char *ctype, const char *body MARGS)
{
	int i;
	...
	*message = NULL;
	...
	i = generating_publish(message, to, from, route MPARAMS);
	if (i != 0) {
...
int eXosip_publish(osip_message_t * message, const char *to MARGS)
{
	...
	i = _eXosip_pub_find_by_aor(&pub, to MPARAMS);
	if (i != 0 || pub == NULL) {
		...
		if (expires == NULL || expires->hvalue == NULL) {
			...
			i = _eXosip_pub_init(&pub, to, expires->hvalue MPARAMS);
			if (i != 0) {
				osip_message_free(message);
				return i;
			}
