...
#else
	osip_transaction_set_your_instance(tr, __eXosip_new_jinfo(jc, jd MPARAMS) );
#endif
...
static int
_eXosip_call_reuse_contact(osip_message_t * invite, osip_message_t * msg MARGS)
{
...
_eXosip_call_transaction_find(int tid, eXosip_call_t ** jc,
							  eXosip_dialog_t ** jd, osip_transaction_t ** tr MARGS)
{
...
int eXosip_call_set_reference(int id, void *reference MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;

	if (id > 0) {
		eXosip_call_dialog_find(id, &jc, &jd MPARAMS);
		if (jc == NULL) {
			eXosip_call_find(id, &jc MPARAMS);
		}
	}
	if (jc == NULL) {
...
void *eXosip_call_get_reference(int cid MARGS)
{
	eXosip_call_t *jc = NULL;
 
	eXosip_call_find(cid, &jc  MPARAMS);
	if (jc == NULL) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					"eXosip: No call here?\n"));
...
eXosip_call_build_initial_invite(osip_message_t ** invite,
								 const char *to,
								 const char *from,
								 const char *route, const char *subject MARGS)
{
	int i;
	osip_to_t *_to = NULL;
	...
	*invite = NULL;
	...
	i = generating_request_out_of_dialog(invite, "INVITE", to,
										 eXosip.transport, from, route MPARAMS);
	osip_to_free(_to);
	if (i != 0)
		return i;
	_eXosip_dialog_add_contact(*invite, NULL MPARAMS);
...
int eXosip_call_send_initial_invite(osip_message_t * invite MARGS)
{
	eXosip_call_t *jc;
	osip_transaction_t *transaction;
	osip_event_t *sipevent;
	...
	i = eXosip_call_init(&jc MPARAMS);
	if (i != 0) {
		osip_message_free(invite);
		return i;
	}

	i = _eXosip_transaction_init(&transaction, ICT, eXosip.j_osip, invite MPARAMS);
	if (i != 0) {
	...
	osip_transaction_set_your_instance(transaction,
									   __eXosip_new_jinfo(jc, NULL, NULL, NULL MPARAMS));
#else
	osip_transaction_set_your_instance(transaction, __eXosip_new_jinfo(jc, NULL MPARAMS));
#endif
	...
	eXosip_update( MPARAMS2);			/* fixed? */
	__eXosip_wakeup( MPARAMS2);
	return jc->c_id;
}

int eXosip_call_build_ack(int did, osip_message_t ** _ack MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	...
	*_ack = NULL;
	...
	if (did <= 0)
		return OSIP_BADPARAMETER;
	if (did > 0) {
		eXosip_call_dialog_find(did, &jc, &jd MPARAMS);
	}
	...
	tr = eXosip_find_last_invite(jc, jd MPARAMS);
	...
	transport = NULL;
	transport = _eXosip_transport_protocol(tr->orig_request MPARAMS);
	if (transport == NULL)
		i = _eXosip_build_request_within_dialog(&ack, "ACK", jd->d_dialog, "UDP" MPARAMS);
	else
	...
	_eXosip_call_reuse_contact(tr->orig_request, ack MPARAMS);
...
int eXosip_call_send_ack(int did, osip_message_t * ack MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	int i;
	...
	if (ack == NULL) {
		i = eXosip_call_build_ack(did, &ack MPARAMS);
		if (i != 0) {
			return i;
		}
	}
	...
	i = cb_snd_message(NULL, ack, host, port, -1 MPARAMS);
...
int
eXosip_call_build_request(int jid, const char *method, osip_message_t ** request MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	...
	*request = NULL;
	...
	if (jid > 0) {
		eXosip_call_dialog_find(jid, &jc, &jd MPARAMS);
	}
	...
	if (0 == osip_strcasecmp(method, "INVITE")) {
		transaction = eXosip_find_last_invite(jc, jd MPARAMS);
	} else {					/* OPTIONS, UPDATE, INFO, REFER, ?... */

		transaction = eXosip_find_last_transaction(jc, jd, method MPARAMS);
	}
	...
	i = _eXosip_build_request_within_dialog(request, method, jd->d_dialog,
											eXosip.transport MPARAMS);
	if (i != 0)
		return i;

	eXosip_add_authentication_information(*request, NULL MPARAMS);

	return OSIP_SUCCESS;
}

int eXosip_call_send_request(int jid, osip_message_t * request MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	...
	if (0 == osip_strcasecmp(request->sip_method, "INVITE")) {
		...
		transaction = eXosip_find_last_transaction(jc, jd, request->sip_method MPARAMS);
	}
	...
	transaction = NULL;
	if (0 != osip_strcasecmp(request->sip_method, "INVITE")) {
		i = _eXosip_transaction_init(&transaction, NICT, eXosip.j_osip, request MPARAMS);
	} else {
		i = _eXosip_transaction_init(&transaction, ICT, eXosip.j_osip, request MPARAMS);
	}
...
int
eXosip_call_build_refer(int did, const char *refer_to, osip_message_t ** request MARGS)
{
	int i;

	*request = NULL;
	i = eXosip_call_build_request(did, "REFER", request MPARAMS);
	if (i != 0)
		return i;
...
int eXosip_call_build_options(int did, osip_message_t ** request MARGS)
{
	int i;

	*request = NULL;
	i = eXosip_call_build_request(did, "OPTIONS", request MPARAMS);
	if (i != 0)
		return i;
...
int eXosip_call_build_info(int did, osip_message_t ** request MARGS)
{
	int i;

	*request = NULL;
	i = eXosip_call_build_request(did, "INFO", request MPARAMS);
	if (i != 0)
		return i;
...
int eXosip_call_build_update(int did, osip_message_t ** request MARGS)
{
	int i;

	*request = NULL;
	i = eXosip_call_build_request(did, "UPDATE", request MPARAMS);
	if (i != 0)
		return i;
						 ...
						 osip_message_t ** request MARGS)
{
	...
	*request = NULL;
	i = eXosip_call_build_request(did, "NOTIFY", request MPARAMS);
	if (i != 0)
		return i;
...
int eXosip_call_build_answer(int tid, int status, osip_message_t ** answer MARGS)
{
	int i = -1;
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	...
	*answer = NULL;
	...
	if (tid > 0) {
		_eXosip_call_transaction_find(tid, &jc, &jd, &tr MPARAMS);
	}
	if (tr == NULL || jd == NULL) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	if (0 == osip_strcasecmp(tr->orig_request->sip_method, "INVITE")) {
		i = _eXosip_answer_invite_123456xx(jc, jd, status, answer, 0 MPARAMS);
	} else {
		...
		if (status > 100 && status < 300)
			i = complete_answer_that_establish_a_dialog(*answer, tr->orig_request MPARAMS);
	}
...
int eXosip_call_send_answer(int tid, int status, osip_message_t * answer MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	osip_transaction_t *tr = NULL;
	...
	if (answer == NULL) {
		if (0 == osip_strcasecmp(tr->orig_request->sip_method, "INVITE")) {
			...
			return _eXosip_answer_invite_123456xx(jc, jd, status, &response, 1 MPARAMS);
		}
		osip_message_free(answer);
		return OSIP_BADPARAMETER;
	}
	...
	if (0 == osip_strcasecmp(tr->orig_request->sip_method, "INVITE")) {
		if (MSG_IS_STATUS_2XX(answer) && jd != NULL) {
			if (status >= 200 && status < 300 && jd != NULL) {
				eXosip_dialog_set_200ok(jd, answer MPARAMS);
				/* wait for a ACK */
...
int eXosip_call_terminate(int cid, int did MARGS)
{
	int i;
	osip_transaction_t *tr;
	...
	if (jd != NULL && jd->d_dialog != NULL
		...
		i = generating_cancel(&request, tr->orig_request MPARAMS);
		if (i != 0) {
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_ERROR, NULL,
		...
		i = eXosip_create_cancel_transaction(jc, jd, request MPARAMS);
		if (i != 0) {
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	if (tr == NULL) {
		...
		if (tr != NULL && tr->last_response != NULL && MSG_IS_STATUS_1XX(tr->last_response)) {	/* answer with 603 */
			...
			i = eXosip_call_send_answer(tr->transactionid, 603, NULL MPARAMS);
	...
	i = generating_bye(&request, jd->d_dialog, eXosip.transport MPARAMS);
	...
	i = eXosip_create_transaction(jc, jd, request MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					"eXosip: cannot initiate SIP transaction!\n"));
...
int eXosip_call_build_prack(int tid, osip_message_t ** prack MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	osip_transaction_t *tr = NULL;
...
int eXosip_call_send_prack(int tid, osip_message_t * prack MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	osip_transaction_t *tr = NULL;
	...
	tr = NULL;
	i = _eXosip_transaction_init(&tr, NICT, eXosip.j_osip, prack MPARAMS);
...
_eXosip_call_retry_request(eXosip_call_t * jc,
						   eXosip_dialog_t * jd, osip_transaction_t * out_tr MARGS)
{
	osip_transaction_t *tr = NULL;
	...
	i = eXosip_update_top_via(msg MPARAMS);
	if (i != 0) {
		osip_message_free(msg);
		return i;
	}
	...
	if (out_tr->last_response->status_code == 401
		...
		eXosip_add_authentication_information(msg, out_tr->last_response MPARAMS);
	else
		eXosip_add_authentication_information(msg, NULL MPARAMS);
	osip_message_force_update(msg);

	if (0 != osip_strcasecmp(msg->sip_method, "INVITE")) {
		i = _eXosip_transaction_init(&tr, NICT, eXosip.j_osip, msg MPARAMS);
	} else {
		i = _eXosip_transaction_init(&tr, ICT, eXosip.j_osip, msg MPARAMS);
	}
...
int eXosip_call_get_referto(int did, char *refer_to, size_t refer_to_len MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	osip_transaction_t *tr = NULL;
...
int eXosip_call_find_by_replaces(char *replaces_str MARGS)
{
	eXosip_call_t *jc = NULL;
	eXosip_dialog_t *jd = NULL;
	char *call_id;
