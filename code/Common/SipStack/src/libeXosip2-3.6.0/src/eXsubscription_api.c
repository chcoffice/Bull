...
int eXosip_subscribe_remove(int did MARGS)
{
	...
	if (did > 0) {
		eXosip_subscribe_dialog_find(did, &js, &jd MPARAMS);
	}
	if (js == NULL) {
...
eXosip_subscribe_build_initial_request(osip_message_t ** sub, const char *to,
									   ...
									   const char *event, int expires MARGS)
{
	char tmp[10];
	int i;
	osip_to_t *_to = NULL;
	...
	*sub = NULL;
	...
	if (i != 0)
		return i;
	_eXosip_dialog_add_contact(*sub, NULL MPARAMS);
...
int eXosip_subscribe_send_initial_request(osip_message_t * subscribe MARGS)
{
	eXosip_subscribe_t *js = NULL;
	osip_transaction_t *transaction;
	...
	i = eXosip_subscribe_init(&js MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	i = _eXosip_transaction_init(&transaction, NICT, eXosip.j_osip, subscribe MPARAMS);
	if (i != 0) {
		eXosip_subscribe_free(js MPARAMS);
		osip_message_free(subscribe);
		return i;
	...
	_eXosip_subscribe_set_refresh_interval(js, subscribe MPARAMS);
	...
	osip_transaction_set_your_instance(transaction,
									   __eXosip_new_jinfo(NULL, NULL, js, NULL MPARAMS));
...
int eXosip_subscribe_build_refresh_request(int did, osip_message_t ** sub MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_subscribe_t *js = NULL;
	...
	*sub = NULL;
	...
	transaction = NULL;
	transaction = eXosip_find_last_out_subscribe(js, jd MPARAMS);
	...
	eXosip_add_authentication_information(*sub, NULL MPARAMS);

	return OSIP_SUCCESS;
}

int eXosip_subscribe_send_refresh_request(int did, osip_message_t * sub MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_subscribe_t *js = NULL;
	...
	transaction = NULL;
	i = _eXosip_transaction_init(&transaction, NICT, eXosip.j_osip, sub MPARAMS);
	...
	js->s_reg_period = 3600;
	_eXosip_subscribe_set_refresh_interval(js, sub MPARAMS);
									...
									osip_transaction_t * out_tr MARGS)
{
	...
	i = eXosip_subscribe_build_refresh_request(jd->d_id, &sub MPARAMS);
	if (i != 0)
		return i;
	...
	i = eXosip_subscribe_send_refresh_request(jd->d_id, sub MPARAMS);
	return i;
}
											   ...
											   osip_transaction_t * out_tr MARGS)
{
	...
	if (out_tr == NULL) {
		out_tr = eXosip_find_last_out_subscribe(js, jd MPARAMS);
	}
	...
	osip_transaction_set_your_instance(tr, __eXosip_new_jinfo(NULL, jd, js, NULL MPARAMS));
