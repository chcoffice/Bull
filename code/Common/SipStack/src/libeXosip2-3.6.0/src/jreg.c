				...
				const char *contact MARGS)
{
	...
	*jr = (eXosip_reg_t *) osip_malloc(sizeof(eXosip_reg_t));
...

	{
		...
		eXosip_generate_random(somerandom, 16  MPARAMS);

		memset(localip, '\0', sizeof(localip));
		memset(firewall_ip, '\0', sizeof(firewall_ip));
		memset(firewall_port, '\0', sizeof(firewall_port));

		eXosip_guess_localip(AF_INET, localip, 128 MPARAMS);
...
void eXosip_reg_free(eXosip_reg_t * jreg MARGS)
{
	...
	if (jreg->r_last_tr != NULL) {
		if (jreg->r_last_tr != NULL && jreg->r_last_tr->orig_request != NULL
			...
			_eXosip_delete_nonce(jreg->r_last_tr->orig_request->call_id->number MPARAMS);
		...
		if (jreg->r_last_tr->state == IST_TERMINATED ||
			...
			__eXosip_delete_jinfo(jreg->r_last_tr MPARAMS);
			if (jreg->r_last_tr != NULL)
