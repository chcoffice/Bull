...
void udp_tl_learn_port_from_via(osip_message_t * sip MARGS);

/* Private functions */
static void rcvregister_failure(osip_transaction_t * tr, osip_message_t * sip MARGS);
...
static int cb_snd_messageEx(osip_transaction_t * tr, osip_message_t * sip,
				   char *host, int port, int out_socket );
static int cb_udp_snd_messageEx(osip_transaction_t * tr, osip_message_t * sip,
					   char *host, int port, int out_socket );
static int cb_tcp_snd_messageEx(osip_transaction_t * tr, osip_message_t * sip,
					   char *host, int port, int out_socket );

#define MY_TRANS_MARK ((void*)0xFFFFFFFF)

#ifdef SIP_INSTANCE_ONE
#define CALLBACK_PARAMS
#else
#define CALLBACK_PARAMS \
eXosip_t *_pXosip =(eXosip_t *)osip_get_application_context((osip_t*)tr->config); \
	if( tr->your_instance == MY_TRANS_MARK) tr=NULL;
#endif
					 ...
					 char *host, int port, int out_socket MARGS )
{
	...
	osip_transaction_t sTr;
	if( !tr)
	{
		memset(&sTr, 0, sizeof(sTr));
		sTr.config = eXosip.j_osip;		
		sTr.your_instance = MY_TRANS_MARK;
		tr = &sTr;	
	}
	
	return cb_snd_messageEx(tr,sip,host,port, out_socket);
}
						 ...
						 char *host, int port, int out_socket  MARGS)
{
	...
	assert(0);
	return -1; //zouyx
	//return cb_udp_snd_messageEx(tr,sip,host,port, out_socket);	
}
						 ...
						 char *host, int port, int out_socket  MARGS)
{
	...
	assert(0);
	return -1; //zouyx
	//return cb_tcp_snd_messageEx(tr,sip,host,port, out_socket);
}

int
cb_snd_messageEx(osip_transaction_t * tr, osip_message_t * sip, char *host,
			   int port, int out_socket)
{
	...
	if (osip_strcasecmp(via->protocol, "udp") == 0) {
		i = eXtl_udp.tl_send_message(tr, sip, host, port, out_socket MPARAMS);
	} else if (osip_strcasecmp(via->protocol, "tcp") == 0) {
		i = eXtl_tcp.tl_send_message(tr, sip, host, port, out_socket MPARAMS);
	}
#ifdef HAVE_OPENSSL_SSL_H
...
static void cb_xixt_kill_transaction(int type, osip_transaction_t * tr)
{
	...
	if (MSG_IS_REGISTER(tr->orig_request)
		...
		rcvregister_failure(tr, NULL MPARAMS);
		return;
	}
#ifndef MINISIZE
	if (type == OSIP_NICT_KILL_TRANSACTION) {
		...
		if (jinfo == NULL && tr->last_response == NULL) {
			eXosip_event_t *je;
			je = eXosip_event_init_for_message(EXOSIP_MESSAGE_REQUESTFAILURE, tr MPARAMS);
		...
		if (jn == NULL && js == NULL) {
			...
			if (jc != NULL && tr->last_response == NULL) {
				report_call_event(EXOSIP_CALL_MESSAGE_REQUESTFAILURE, jc, jd, tr MPARAMS);
				return;
			}
		...
		if (js != NULL && MSG_IS_SUBSCRIBE(tr->orig_request)
			...
			je = eXosip_event_init_for_subscribe
				(EXOSIP_SUBSCRIPTION_REQUESTFAILURE, js, jd, tr MPARAMS);
			report_event(je, NULL MPARAMS);
	...
	if ((jinfo->jd != NULL) && (jinfo->jc != NULL))
	  {
		  ...
		  report_call_event (EXOSIP_CALL_CANCELLED, jinfo->jc, jinfo->jd, tr MPARAMS);
	  }
}
...
static void cb_rcvregister(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	//je = eXosip_event_init_for_message(EXOSIP_MESSAGE_NEW, tr MPARAMS);
	je = eXosip_event_init_for_message(EXOSIP_REGISTRATION_NEW, tr MPARAMS);
...
static void cb_rcvrequest(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (jc == NULL) {
		...
		if (MSG_IS_BYE(sip)) {
			...
			report_call_event(EXOSIP_CALL_MESSAGE_NEW, jc, jd, tr MPARAMS);
		return;
	}
#ifndef MINISIZE
	else if (jn != NULL) {
...
static void cb_rcv1xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	udp_tl_learn_port_from_via(sip MPARAMS);
	...
	if (MSG_IS_RESPONSE_FOR(sip, "OPTIONS")) {
		if (jc == NULL) {
			...
			je = eXosip_event_init_for_message(EXOSIP_MESSAGE_PROCEEDING, tr MPARAMS);
			eXosip_event_add(je MPARAMS);
			return;
		}
		report_call_event(EXOSIP_CALL_MESSAGE_PROCEEDING, jc, jd, tr MPARAMS);
		return;
	}

	if (MSG_IS_RESPONSE_FOR(sip, "INVITE") && MSG_TEST_CODE(sip, 100)) {
		report_call_event(EXOSIP_CALL_PROCEEDING, jc, jd, tr MPARAMS);
	}
	...
	if ((MSG_IS_RESPONSE_FOR(sip, "INVITE")
		...
		if (jd == NULL) {		/* This transaction initiate a dialog in the case of
			...
			i = eXosip_dialog_init_as_uac(&jd, sip MPARAMS);
			if (i != 0) {
				OSIP_TRACE(osip_trace
		...
		if (jd != NULL && MSG_IS_RESPONSE_FOR(sip, "INVITE")
			...
			report_call_event(EXOSIP_CALL_PROCEEDING, jc, jd, tr MPARAMS);
				   ...
				   && sip->status_code >= 180) {
			report_call_event(EXOSIP_CALL_RINGING, jc, jd, tr MPARAMS);
		}
#ifndef MINISIZE
		...
		if (jc != NULL && MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
			eXosip_call_renew_expire_time(jc MPARAMS);
        }
	}
}
...
static void cb_rcv2xx_4invite(osip_transaction_t * tr, osip_message_t * sip)
{
	...
	report_call_event(EXOSIP_CALL_ANSWERED, jc, jd, tr MPARAMS);
...
static void cb_rcv2xx_4subscribe(osip_transaction_t * tr, osip_message_t * sip)
{
	...
	jd = jinfo->jd;
	js = jinfo->js;
	_eXosip_subscribe_set_refresh_interval(js, sip MPARAMS);
...
static void cb_rcv2xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "PUBLISH")) {
		...
		i = _eXosip_pub_update(&pub, tr, sip MPARAMS);
		if (i != 0) {
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_ERROR, NULL,
		...
		je = eXosip_event_init_for_message(EXOSIP_MESSAGE_ANSWERED, tr MPARAMS);
		report_event(je, sip MPARAMS);
		return;
	} else
#endif
	if (MSG_IS_RESPONSE_FOR(sip, "REGISTER")) {
		...
		/* find matching j_reg */
		_eXosip_reg_find(&jreg, tr MPARAMS);
		if (jreg != NULL) {
			/* update registration interval */
			osip_header_t *exp;
			...
			je = eXosip_event_init_for_reg(EXOSIP_REGISTRATION_SUCCESS, jreg, tr MPARAMS);
			report_event(je, sip MPARAMS);
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		...
		report_call_event(EXOSIP_CALL_MESSAGE_ANSWERED, jc, jd, tr MPARAMS);
	}
#ifndef MINISIZE
...
void eXosip_delete_early_dialog(eXosip_dialog_t * jd MARGS)
{
	...
	if (jd->d_dialog != NULL && jd->d_dialog->state == DIALOG_EARLY) {
		...
		eXosip_dialog_set_state(jd, JD_TERMINATED MPARAMS);
	}
}
...
static void rcvregister_failure(osip_transaction_t * tr, osip_message_t * sip MARGS)
{
	...
	_eXosip_reg_find(&jreg, tr MPARAMS);
	if (jreg != NULL) {
		je = eXosip_event_init_for_reg(EXOSIP_REGISTRATION_FAILURE, jreg, tr MPARAMS);
		report_event(je, sip MPARAMS);
	}
...
static void cb_rcv3xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "PUBLISH")) {
		...
		je = eXosip_event_init_for_message(EXOSIP_MESSAGE_REDIRECTED, tr MPARAMS);
		report_event(je, sip MPARAMS);
		return;
	} else if (MSG_IS_RESPONSE_FOR(sip, "REGISTER")) {
		rcvregister_failure(tr, sip MPARAMS);
		return;
	}
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		report_call_event(EXOSIP_CALL_REDIRECTED, jc, jd, tr MPARAMS);
	} else if (MSG_IS_RESPONSE_FOR(sip, "NOTIFY") && jn != NULL) {
		eXosip_event_t *je;
		...
		report_call_event(EXOSIP_CALL_MESSAGE_REDIRECTED, jc, jd, tr MPARAMS);
		return;
	} else if (jc == NULL && js == NULL && jn == NULL) {
		eXosip_event_t *je;
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")
		|| MSG_IS_RESPONSE_FOR(sip, "SUBSCRIBE")) {
		eXosip_delete_early_dialog(jd MPARAMS);
		if (jd->d_dialog == NULL)
...
static void cb_rcv4xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		report_call_event(EXOSIP_CALL_REQUESTFAILURE, jc, jd, tr MPARAMS);
	} else if (MSG_IS_RESPONSE_FOR(sip, "NOTIFY") && jn != NULL) {
		eXosip_event_t *je;
...
static void cb_rcv5xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "PUBLISH")) {
		...
		je = eXosip_event_init_for_message(EXOSIP_MESSAGE_SERVERFAILURE, tr MPARAMS);
		report_event(je, sip MPARAMS);
		return;
	} else if (MSG_IS_RESPONSE_FOR(sip, "REGISTER")) {
		rcvregister_failure(tr, sip MPARAMS);
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		report_call_event(EXOSIP_CALL_SERVERFAILURE, jc, jd, tr MPARAMS);
	} else if (MSG_IS_RESPONSE_FOR(sip, "NOTIFY") && jn != NULL) {
		eXosip_event_t *je;
		...
		report_call_event(EXOSIP_CALL_MESSAGE_SERVERFAILURE, jc, jd, tr MPARAMS);
		return;
	} else if (jc == NULL && js == NULL && jn == NULL) {
		eXosip_event_t *je;
...
static void cb_rcv6xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "PUBLISH")) {
		...
		je = eXosip_event_init_for_message(EXOSIP_MESSAGE_GLOBALFAILURE, tr MPARAMS);
		report_event(je, sip MPARAMS);
		return;
	} else if (MSG_IS_RESPONSE_FOR(sip, "REGISTER")) {
		rcvregister_failure(tr, sip MPARAMS);
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		report_call_event(EXOSIP_CALL_GLOBALFAILURE, jc, jd, tr MPARAMS);
	} else if (MSG_IS_RESPONSE_FOR(sip, "NOTIFY") && jn != NULL) {
		eXosip_event_t *je;
		...
		report_call_event(EXOSIP_CALL_MESSAGE_GLOBALFAILURE, jc, jd, tr MPARAMS);
		return;
	} else if (jc == NULL && js == NULL && jn == NULL) {
		eXosip_event_t *je;
...
static void cb_snd123456xx(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (MSG_IS_RESPONSE_FOR(sip, "INVITE")) {
		...
		if (jc != NULL && tr == jc->c_inc_tr) {
			report_call_event(EXOSIP_CALL_CLOSED, jc, jd, tr MPARAMS);
		}
	}
...
int eXosip_set_callbacks(osip_t * osip MARGS)
{
	...
	osip_set_cb_send_message(osip, &cb_snd_messageEx);
