...
/* Private functions */
static int dialog_fill_route_set(osip_dialog_t * dialog, osip_message_t * request MARGS);
...
{
	return osip_call_id_new_random( MPARAMS2);
}
...
int _eXosip_dialog_add_contact(osip_message_t * request, osip_message_t * answer MARGS)
{
	osip_via_t *via;
	osip_from_t *a_from;
	char *contact = NULL;
	...
	if (locip[0] == '\0') {
		eXosip_guess_ip_for_via(eXosip.eXtl->proto_family, locip, 49 MPARAMS);
		if (locip[0] == '\0') {
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_ERROR, NULL,
						...
						const char *locip MARGS)
{
	...
	if (eXosip.eXtl->proto_family == AF_INET6)
		snprintf(tmp, 200, "SIP/2.0/%s [%s]:%s;branch=z9hG4bK%u",
				 eXosip.transport, ip, firewall_port, via_branch_new_random( MPARAMS2));
	else {
								 ...
								 const char *from, const char *proxy MARGS)
{
	...
	*dest = NULL;
	...
	osip_uri_param_t  *pParam = NULL;
	if( osip_from_get_tag(request->from, &pParam ) || pParam == NULL)
	{
		osip_from_set_tag(request->from, osip_from_tag_new_random( MPARAMS2));
	}
	}
	...
	/* set the cseq and call_id header */
	{
		...
		cidrand = osip_call_id_new_random( MPARAMS2);
	...
	i = _eXosip_request_add_via(request, transport, locip MPARAMS);
	if (i != 0) {
		osip_message_free(request);
		return i;
	}
...
generating_register(eXosip_reg_t * jreg, osip_message_t ** reg, char *transport,
					char *from, char *proxy, char *contact, int expires MARGS)
{
	int i;
	char locip[65];
	char firewall_ip[65];
...
_eXosip_build_request_within_dialog(osip_message_t ** dest,
									const char *method,
									osip_dialog_t * dialog, const char *transport MARGS)
{
	int i;
	osip_message_t *request;
	char locip[65];
	...
	*dest = NULL;
	...
	if (osip_list_eol(&dialog->route_set, 0)) {
		...
		i = dialog_fill_route_set(dialog, request MPARAMS);
		if (i != 0) {
			osip_message_free(request);
			return i;
		}
...
#else
	_eXosip_dialog_add_contact(request, NULL MPARAMS);
#endif
...
int generating_bye(osip_message_t ** bye, osip_dialog_t * dialog, char *transport MARGS)
{
	int i;

	i = _eXosip_build_request_within_dialog(bye, "BYE", dialog, transport MPARAMS);
	if (i != 0)
		return i;
...
int generating_cancel(osip_message_t ** dest, osip_message_t * request_cancelled MARGS)
{
	int i;
	osip_message_t *request;
