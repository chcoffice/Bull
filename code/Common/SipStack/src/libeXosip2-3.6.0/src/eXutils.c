...
void eXosip_freeaddrinfo(struct addrinfo *ai MARGS)
{
...
static struct addrinfo *osip_he2ai(struct hostent *he, int port, int protocol MARGS)
{
	struct addrinfo *ai;
...
static int eXosip_inet_pton(int family, const char *src, void *dst MARGS)
{
...
eXosip_get_addrinfo(struct addrinfo **addrinfo,
					const char *hostname, int port, int protocol  MARGS)
{
...
int eXosip_guess_ip_for_via(int family, char *address, int size  MARGS)
{
	...
	if (family == AF_INET) {
		eXosip_get_addrinfo(&addrf, eXosip.ipv4_for_gateway, 0, IPPROTO_UDP MPARAMS);
	} else if (family == AF_INET6) {
		eXosip_get_addrinfo(&addrf, eXosip.ipv6_for_gateway, 0, IPPROTO_UDP MPARAMS);
	}
...
int
_eXosip_srv_lookup(osip_message_t * sip, osip_naptr_t **naptr_record   MARGS) 
{
	int use_srv = 1;
...
int eXosip_dnsutils_rotate_srv(osip_srv_record_t *srv_record   MARGS)
{
	int n;
...
int eXosip_dnsutils_dns_process(osip_naptr_t *naptr_record, int force   MARGS)
{
	return OSIP_SUCCESS;
}
