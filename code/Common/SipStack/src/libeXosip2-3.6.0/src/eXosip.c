						 ...
						 osip_message_t * message  MARGS)
{
	...
	}
	{
		...
		i = _eXosip_srv_lookup(message, &naptr_record MPARAMS);
		if (i < 0) {
						...
						int *retry  MARGS)
{
	...
	if (out_tr != NULL && out_tr->last_response != NULL
		...
		i = eXosip_add_authentication_information(msg, out_tr->last_response  MPARAMS);
		if (i < 0) {
			osip_message_free(msg);
			return i;
		}
	} else {
		i = eXosip_add_authentication_information(msg, NULL  MPARAMS);
		if (i < 0) {
			osip_message_free(msg);
			return i;
		}
...
static int _eXosip_retry_register_with_auth(eXosip_event_t * je  MARGS)
{
	eXosip_reg_t *jr = NULL;
	int i;

	i = eXosip_reg_find_id(&jr, je->rid MPARAMS);
	if (i < 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	if (jr->r_retry < 3) {
		jr->r_retry++;
		return eXosip_register_send_register(jr->r_id, NULL MPARAMS);
	}

	return OSIP_UNDEFINED_ERROR;
}

static int _eXosip_retry_invite_with_auth(eXosip_event_t * je  MARGS)
{
	...
	i = _eXosip_call_transaction_find(je->tid, &jc, &jd, &tr MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	if (*retry < 3) {
		(*retry)++;
		return _eXosip_call_retry_request(jc, jd, tr MPARAMS);
	}
	return OSIP_UNDEFINED_ERROR;
}

static int _eXosip_redirect_invite(eXosip_event_t * je  MARGS)
{
	eXosip_dialog_t *jd = NULL;
	eXosip_call_t *jc = NULL;
	osip_transaction_t *tr = NULL;
...
static int _eXosip_retry_subscribe_with_auth(eXosip_event_t * je  MARGS)
{
	eXosip_dialog_t *jd = NULL;
	...
	i = _eXosip_subscribe_transaction_find(je->tid, &js, &jd, &tr MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	if (*retry < 3) {
		(*retry)++;
		return _eXosip_subscribe_send_request_with_credential(js, jd, tr MPARAMS);
	}
	return OSIP_UNDEFINED_ERROR;
}

static int _eXosip_retry_publish_with_auth(eXosip_event_t * je  MARGS)
{
	eXosip_pub_t *jp = NULL;
	int i;

	i = _eXosip_pub_find_by_tid(&jp, je->tid MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	return _eXosip_retry_with_auth(NULL, &jp->p_last_tr, NULL MPARAMS);
}

static int _eXosip_retry_notify_with_auth(eXosip_event_t * je  MARGS)
{
	/* TODO untested */
	eXosip_dialog_t *jd = NULL;
	...
	i = _eXosip_insubscription_transaction_find(je->tid, &jn, &jd, &tr MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	return _eXosip_insubscription_send_request_with_credential(jn, jd, tr MPARAMS);

}

static int eXosip_retry_with_auth(eXosip_event_t * je  MARGS)
{
	...
	if (je->rid > 0) {
		return _eXosip_retry_register_with_auth(je MPARAMS);
	} else if (je->cid > 0) {
		res = _eXosip_retry_invite_with_auth(je MPARAMS);
		if (res==OSIP_NOTFOUND)
		{
			osip_transaction_t *tr = NULL;
			eXosip_transaction_find(je->tid, &tr MPARAMS);
			if (tr != NULL) {
				return _eXosip_retry_with_auth(NULL, &tr, NULL MPARAMS);
			}
		}
		return res;
	}
		...
		return _eXosip_retry_subscribe_with_auth(je MPARAMS);
	} else if (je->nid > 0) {
		return _eXosip_retry_notify_with_auth(je MPARAMS);
	} else if (MSG_IS_PUBLISH(je->request))
		return _eXosip_retry_publish_with_auth(je MPARAMS);
#endif
	else {
		osip_transaction_t *tr = NULL;
		eXosip_transaction_find(je->tid, &tr MPARAMS);
...
static int _eXosip_redirect(eXosip_event_t * je  MARGS)
{
	switch (je->type) {
	case EXOSIP_CALL_REDIRECTED:
		return _eXosip_redirect_invite(je MPARAMS);
...
int eXosip_default_action(eXosip_event_t * je  MARGS)
{
	if (!je || !je->response)
		return OSIP_BADPARAMETER;

	if (je->response->status_code == 401 || je->response->status_code == 407)
		return eXosip_retry_with_auth(je MPARAMS);
	else if (je->response->status_code >= 300 && je->response->status_code <= 399)
		return _eXosip_redirect(je MPARAMS);
	else
		return 1;
}

void eXosip_automatic_refresh(  MARGS2)
{
		...
		for (jd = js->s_dialogs; jd != NULL; jd = jd->next) {
			if (jd->d_dialog != NULL && (jd->d_id >= 1)) {	/* finished call */
				...
				if (js->s_reg_period == 0 || out_tr == NULL) {
					...
					i = _eXosip_subscribe_automatic_refresh(js, jd, out_tr MPARAMS);
					if (i != 0) {
						OSIP_TRACE(osip_trace
								   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	for (jr = eXosip.j_reg; jr != NULL; jr = jr->next) {
		...
		if (jr->r_id >= 1 && jr->r_last_tr != NULL) {
			if (jr->r_reg_period == 0) {
				...
				/* automatic refresh */
				eXosip_register_send_register(jr->r_id, NULL MPARAMS);
#if TARGET_OS_IPHONE
			...
			for (jd = jc->c_dialogs; jd != NULL; jd = jd->next) {
				if (jd->d_id >= 1 && jd->d_dialog != NULL && jd->d_200Ok != NULL) {
					if (jd->d_count == 9) {
						...
						jd->d_200Ok = NULL;
						i = eXosip_call_terminate(jc->c_id, jd->d_id MPARAMS);
						if (i == OSIP_SUCCESS)
						{
							report_call_event(EXOSIP_CALL_CLOSED, jc, jd, NULL MPARAMS);
						}
						...
						jd = jc->c_dialogs;
						/* TU retransmission */
						cb_snd_message(NULL, jd->d_200Ok, NULL, 0, -1 MPARAMS);
					}
				}
			}
		}
...
void eXosip_automatic_action(  MARGS2)
{
	eXosip_call_t *jc;
	eXosip_dialog_t *jd;
#ifndef MINISIZE
		...
		if (jc->c_id < 1) {
			...
			if (out_tr != NULL
				...
				if (jc->c_retry < 3) {
					...
					i = _eXosip_call_retry_request(jc, NULL, out_tr MPARAMS);
					if (i != 0) {
						OSIP_TRACE(osip_trace
								   (__FILE__, __LINE__, OSIP_ERROR, NULL,
									"eXosip: could not clone msg for authentication\n"));
			...
			if (jd->d_dialog == NULL) {	/* finished call */
				...
				if (out_tr != NULL
					...
					if (jd->d_retry < 3) {
						...
						i = _eXosip_call_retry_request(jc, jd, out_tr MPARAMS);
						if (i != 0) {
							OSIP_TRACE(osip_trace
									   (__FILE__, __LINE__, OSIP_ERROR, NULL,
										"eXosip: could not clone msg for authentication\n"));
					...
					if (jd->d_refresher == -1) {
						...
						if (now -
							...
							eXosip_call_build_update(jd->d_id, &request MPARAMS);
							if (request != NULL) {
								char session_exp[32];
								int i;
								...
								i = eXosip_call_send_request(jd->d_id, request MPARAMS);
								if (i == 0) {
									/* update timer */
									jd->d_session_timer_start =
	...
	for (jpub = eXosip.j_pub; jpub != NULL; jpub = jpub->next) {
		if (jpub->p_id >= 1 && jpub->p_last_tr != NULL) {
			if (jpub->p_period != 0 && now - jpub->p_last_tr->birth_time > 900) {
				/* automatic refresh */
				_eXosip_publish_refresh(NULL, &jpub->p_last_tr, NULL MPARAMS);
					   ...
					   && now - jpub->p_last_tr->birth_time > jpub->p_period - (jpub->p_period/10))
			{
				...
				if (jpub->p_retry < 3) {
					/* TODO: improve support for several retries when
					   several credentials are needed */
					_eXosip_retry_with_auth(NULL, &jpub->p_last_tr, NULL MPARAMS);
					jpub->p_retry++;
				}
			} else if (now - jpub->p_last_tr->birth_time < 120 &&
					   jpub->p_last_tr->orig_request != NULL &&
...
int eXosip_clear_authentication_info(  MARGS2 )
{
									  ...
									  osip_message_t * last_response  MARGS)
{
	...
	while (wwwauth != NULL) {
		...
		i = __eXosip_create_authorization_header(wwwauth, uri,
												 ...
												 authinfo->passwd,
												 authinfo->ha1, &aut,
												 req->sip_method, "0a4f113b", 1 MPARAMS);
		osip_free(uri);
		if (i != 0)
			return i;
		...
		if (wwwauth->qop_options != NULL) {
			...
			if (osip_strcasecmp(req->sip_method, "REGISTER") == 0
				...
				_eXosip_store_nonce(req->call_id->number, wwwauth, 401 MPARAMS);
			else {
	...
	while (proxyauth != NULL) {
		...
		if (proxyauth->qop_options != NULL) {
			...
			if (osip_strcasecmp(req->sip_method, "REGISTER") == 0
				|| osip_strcasecmp(req->sip_method, "INVITE") == 0
				|| osip_strcasecmp(req->sip_method, "SUBSCRIBE") == 0)
				_eXosip_store_nonce(req->call_id->number, proxyauth, 407 MPARAMS);
			else {
				osip_generic_param_t *to_tag = NULL;
				osip_from_param_get_byname(req->to, "tag", &to_tag);
				if (to_tag != NULL) {
