...
#define eXosip_freeaddrinfo freeaddrinfo
	...
	void eXosip_update( MARGS2 );
#ifdef OSIP_MT
	void __eXosip_wakeup( MARGS2 );
#else
#define __eXosip_wakeup( MARGS2 )  ;
#endif
...
#define __eXosip_sockaddr sockaddr_storage
	...
	int _eXosip_pub_update(eXosip_pub_t ** pub, osip_transaction_t * tr,
						   osip_message_t * answer MARGS);
	int _eXosip_pub_find_by_aor(eXosip_pub_t ** pub, const char *aor MARGS);
	int _eXosip_pub_find_by_tid(eXosip_pub_t ** pjp, int tid MARGS);
	int _eXosip_pub_init(eXosip_pub_t ** pub, const char *aor, const char *exp MARGS);
	void _eXosip_pub_free(eXosip_pub_t * pub MARGS);
	...
	int
	 __eXosip_create_authorization_header(osip_www_authenticate_t * wa,
										  ...
										  const char *method,
										  const char *pszCNonce, int iNonceCount MARGS);
	...
	int eXosip_event_init(eXosip_event_t ** je, int type MARGS);
	void report_call_event(int evt, eXosip_call_t * jc, eXosip_dialog_t * jd,
						   osip_transaction_t * tr MARGS);
	void report_event(eXosip_event_t * je, osip_message_t * sip MARGS);
	int eXosip_event_add(eXosip_event_t * je MARGS);
	eXosip_event_t *eXosip_event_wait(int tv_s, int tv_ms MARGS);
	eXosip_event_t *eXosip_event_get( MARGS2 );
...
#define EXOSIP_MAX_SOCKETS 200
	...
	char *_eXosip_transport_protocol(osip_message_t * msg MARGS);
	int _eXosip_find_protocol(osip_message_t * msg MARGS);
	int _eXosip_tcp_find_socket(char *host, int port MARGS);
	int _eXosip_tcp_connect_socket(char *host, int port MARGS);
	int setsockopt_ipv6only(int sock MARGS);
...
#define MAX_EXOSIP_HTTP_AUTH 100
	...
	struct eXosip_t {
		...
		struct eXtl_protocol xtl_udp;
		struct eXtl_protocol xtl_tcp;
#ifndef DISABLE_TLS
		struct eXtl_protocol xtl_tls;
		struct eXtl_protocol xtl_dtls;
#endif
	...
	int eXosip_guess_ip_for_via(int family, char *address, int size MARGS);
 ...
 */
	int eXosip_get_addrinfo(struct addrinfo **addrinfo, const char *hostname,
							int service, int protocol MARGS);

	int eXosip_set_callbacks(osip_t * osip MARGS);
	...
	int cb_tcp_snd_message(osip_transaction_t * tr, osip_message_t * sip,
						   char *host, int port, int out_socket  MARGS);
	char *osip_call_id_new_random(MARGS2);
	char *osip_to_tag_new_random(MARGS2);
	char *osip_from_tag_new_random(MARGS2);
	unsigned int via_branch_new_random(MARGS2);
	void __eXosip_delete_jinfo(osip_transaction_t * transaction MARGS);
#ifndef MINISIZE
	jinfo_t *__eXosip_new_jinfo(eXosip_call_t * jc, eXosip_dialog_t * jd,
								eXosip_subscribe_t * js, eXosip_notify_t * jn MARGS);
#else
	jinfo_t *__eXosip_new_jinfo(eXosip_call_t * jc, eXosip_dialog_t * jd MARGS);
#endif

	int eXosip_dialog_init_as_uac(eXosip_dialog_t ** jd, osip_message_t * _200Ok MARGS);
	int eXosip_dialog_init_as_uas(eXosip_dialog_t ** jd,
								  osip_message_t * _invite,
								  osip_message_t * _200Ok MARGS);
	void eXosip_dialog_free(eXosip_dialog_t * jd MARGS);
	void eXosip_dialog_set_state(eXosip_dialog_t * jd, int state MARGS);
	void eXosip_delete_early_dialog(eXosip_dialog_t * jd MARGS);


	int isrfc1918(char *ipaddr MARGS);
	void eXosip_get_localip_from_via(osip_message_t *, char *localip, int size MARGS );
	int generating_request_out_of_dialog(osip_message_t ** dest,
										 const char *method, const char *to,
										 const char *transport,
										 const char *from, const char *proxy MARGS);
	int generating_publish(osip_message_t ** message, const char *to,
						   const char *from, const char *route MARGS);
	int generating_cancel(osip_message_t ** dest,
						  osip_message_t * request_cancelled MARGS);
	int generating_bye(osip_message_t ** bye, osip_dialog_t * dialog,
					   char *transport MARGS);

	int eXosip_update_top_via(osip_message_t * sip MARGS);
	int _eXosip_request_add_via(osip_message_t * request, const char *transport,
								const char *locip MARGS);

	void eXosip_mark_all_registrations_expired (MARGS2);

	int eXosip_add_authentication_information(osip_message_t * req,
											  osip_message_t * last_response MARGS);
	int _eXosip_reg_find(eXosip_reg_t ** reg, osip_transaction_t * tr MARGS);
	int eXosip_reg_find_id(eXosip_reg_t ** reg, int rid MARGS);
	int eXosip_reg_init(eXosip_reg_t ** jr, const char *from,
						const char *proxy, const char *contact MARGS);
	void eXosip_reg_free(eXosip_reg_t * jreg MARGS);
	int generating_register(eXosip_reg_t * jreg, osip_message_t ** reg,
							char *transport, char *from, char *proxy,
							char *contact, int expires MARGS);
	...
	int _eXosip_call_retry_request(eXosip_call_t * jc,
								   eXosip_dialog_t * jd,
								   osip_transaction_t * out_tr MARGS);
	int eXosip_transaction_find(int tid, osip_transaction_t ** transaction MARGS);
	...
	int eXosip_subscribe_dialog_find(int nid, eXosip_subscribe_t ** js,
									 eXosip_dialog_t ** jd MARGS);
#endif
	int eXosip_call_find(int cid, eXosip_call_t ** jc MARGS);
	int eXosip_dialog_set_200ok(eXosip_dialog_t * _jd, osip_message_t * _200Ok MARGS);
	...
	int _eXosip_insubscription_answer_1xx(eXosip_notify_t * jc,
										  eXosip_dialog_t * jd, int code MARGS);
	...
	int eXosip_build_response_default(int jid, int status MARGS);
	...
	int _eXosip_build_request_within_dialog(osip_message_t ** dest,
											...
											const char *transport MARGS);
	void eXosip_kill_transaction(osip_list_t * transactions MARGS);
	int eXosip_remove_transaction_from_call(osip_transaction_t * tr,
											eXosip_call_t * jc MARGS);
#ifndef MINISIZE
	...
	int eXosip_call_init(eXosip_call_t ** jc MARGS);
	void eXosip_call_renew_expire_time(eXosip_call_t * jc MARGS);
	void eXosip_call_free(eXosip_call_t * jc MARGS);
	...
	int eXosip_subscribe_init(eXosip_subscribe_t ** js MARGS);
	void eXosip_subscribe_free(eXosip_subscribe_t * js MARGS);
	int _eXosip_subscribe_set_refresh_interval(eXosip_subscribe_t * js,
											   osip_message_t * inc_subscribe MARGS);
	int eXosip_subscribe_need_refresh(eXosip_subscribe_t * js,
									  eXosip_dialog_t * jd, int now MARGS);
	...
	int _eXosip_subscribe_automatic_refresh(eXosip_subscribe_t * js,
											eXosip_dialog_t * jd,
											osip_transaction_t * out_tr MARGS);
	int eXosip_notify_init(eXosip_notify_t ** jn, osip_message_t * inc_subscribe MARGS);
	void eXosip_notify_free(eXosip_notify_t * jn MARGS);
	int _eXosip_notify_set_contact_info(eXosip_notify_t * jn, char *uri MARGS);
	...
	int eXosip_is_public_address(const char *addr MARGS);

	void eXosip_retransmit_lost200ok(MARGS2);
	...
	int _eXosip_transaction_init(osip_transaction_t ** transaction,
								 ...
								 osip_message_t * message MARGS);

	int _eXosip_srv_lookup(osip_message_t * sip, osip_naptr_t **naptr_record MARGS);

	void _eXosip_dnsutils_release(osip_naptr_t *naptr_record MARGS);
