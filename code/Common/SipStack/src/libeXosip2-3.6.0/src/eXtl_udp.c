...
#else
#define udp_socket eXosip.xtl_udp.socket
#define my_addr eXosip.xtl_udp.addr

#define udp_firewall_ip eXosip.xtl_udp.firewall_ip
#define udp_firewall_port eXosip.xtl_udp.firewall_port
#endif

static int udp_tl_init(MARGS2)
{
	udp_socket = 0;
...
static int udp_tl_open( MARGS2 )
{
	...
	res = eXosip_get_addrinfo(&addrinfo,
							  eXtl_udp.proto_ifs,
							  eXtl_udp.proto_port, eXtl_udp.proto_num MPARAMS);
	if (res)
		return -1;
...
static int udp_tl_set_fdset(fd_set * osip_fdset, fd_set * osip_wrset, int *fd_max MARGS)
{
	if (udp_socket <= 0)
		return -1;
		...
		if (i >= 0 && via != NULL && via->protocol != NULL
			...
			if (br != NULL && br->gvalue != NULL) {
				...
				if (br != NULL && br->gvalue != NULL
					...
					eXosip_set_option(EXOSIP_OPT_ADD_ACCOUNT_INFO, &ainfo MPARAMS);
				}
			}
		}
	}
...
static int udp_tl_read_message(fd_set * osip_fdset, fd_set * osip_wrset MARGS)
{
	char *buf;
	int i;
...
static int eXtl_update_local_target(osip_message_t * req MARGS)
{
	int pos = 0;
					...
					int port, int out_socket MARGS)
{
	...
	eXtl_update_local_target(sip MPARAMS);
					...
					*/
					...
					for (srv = &naptr_record->sipudp_record.srventry[naptr_record->sipudp_record.index];
						...
						srv = &naptr_record->sipudp_record.srventry[naptr_record->sipudp_record.index]) {
							if (srv->ipaddress[0])
								i = eXosip_get_addrinfo(&addrinfo, srv->ipaddress, srv->port, IPPROTO_UDP MPARAMS);
							else
								i = eXosip_get_addrinfo(&addrinfo, srv->srv, srv->port, IPPROTO_UDP MPARAMS);
							if (i == 0) {
								host = srv->srv;
								port = srv->port;
								break;
							...
							i = eXosip_dnsutils_rotate_srv(&naptr_record->sipudp_record MPARAMS);
							if (i<=0)
							{
								return -1;
							}
	...
	if (i != 0) {
		i = eXosip_get_addrinfo(&addrinfo, host, port, IPPROTO_UDP MPARAMS);
	}
	...
	if (osip_strcasecmp(host, ipbuf)!=0 && MSG_IS_REQUEST(sip)) {
		if (MSG_IS_REGISTER(sip)) {
			...
			eXosip_set_option(EXOSIP_OPT_ADD_DNS_CACHE, (void *) &entry MPARAMS);
		}
	}
			   ...
			   (struct sockaddr *) &addr, len))
	{
		...
		{
			/* rotate on failure! */
			if (eXosip_dnsutils_rotate_srv(&naptr_record->sipudp_record MPARAMS)>0)
			{
				osip_free(message);
	...
	if (eXosip.keep_alive > 0) {
		if (MSG_IS_REGISTER(sip)) {
			...
			if (_eXosip_reg_find(&reg, tr MPARAMS) == 0) {
...
static int udp_tl_keepalive(MARGS2)
{
...
static int udp_tl_set_socket(int socket MARGS)
{
	udp_socket = socket;
...
static int udp_tl_masquerade_contact(const char *public_address, int port MARGS)
{
...
static int
udp_tl_get_masquerade_contact(char *ip, int ip_size, char *port, int port_size MARGS)
{
	memset(ip, 0, ip_size);
...
#else
static struct eXtl_protocol Default_eXtl_udp = {
	1,
	15060,
	"UDP",
	"0.0.0.0",
...
#else
	memcpy(eXtl, &Default_eXtl_udp,sizeof(*eXtl));
	Default_eXtl_udp.proto_port++;
#endif
}
