	...
	*dest = NULL;
	...
	if (i != 0) {				/* we only add a tag if it does not already contains one! */
			...
			/* it should contain the local TAG we created */
		{
			...
			if (status != 100)
				osip_to_set_tag(response->to, osip_to_tag_new_random( MPARAMS2));
		}
	}
...
_eXosip_answer_invite_123456xx(eXosip_call_t * jc, eXosip_dialog_t * jd, int code,
							   osip_message_t ** answer, int send MARGS)
{
	int i;
	...
	*answer = NULL;
	...
	if (jd == NULL)
		i = _eXosip_build_response_default(answer, NULL, code, tr->orig_request MPARAMS);
	else
								  ...
								  int code MARGS)
{
	...
	tr = eXosip_find_last_inc_subscribe(jn, jd MPARAMS);
	if (tr == NULL) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
					"eXosip: cannot find transaction to answer"));
	...
	if (code > 100) {
		/* request that estabish a dialog: */
		/* 12.1.1 UAS Behavior */
		i = complete_answer_that_establish_a_dialog(response, tr->orig_request MPARAMS);
		if (i != 0) {
		}

		if (jd == NULL) {
			i = eXosip_dialog_init_as_uas(&jd, tr->orig_request, response MPARAMS);
			if (i != 0) {
				OSIP_TRACE(osip_trace
						   (__FILE__, __LINE__, OSIP_ERROR, NULL,
