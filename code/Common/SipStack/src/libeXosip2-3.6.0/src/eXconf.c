...
#endif
static void _eXosip_keep_alive( MARGS2 );

#ifndef MINISIZE

void eXosip_enable_ipv6(int _ipv6_enable MARGS)
{
...
int eXosip_set_cbsip_message(CbSipCallback cbsipCallback  MARGS)
{
	eXosip.cbsipCallback = cbsipCallback;
	return 0;
}

void eXosip_masquerade_contact(const char *public_address, int port  MARGS)
{
	eXtl_udp.tl_masquerade_contact(public_address, port MPARAMS);
	eXtl_tcp.tl_masquerade_contact(public_address, port MPARAMS);
#ifdef HAVE_OPENSSL_SSL_H
	eXtl_tls.tl_masquerade_contact(public_address, port MPARAMS);
#if !(OPENSSL_VERSION_NUMBER < 0x00908000L)
	eXtl_dtls.tl_masquerade_contact(public_address, port MPARAMS);
#endif
#endif
	return;
}

int eXosip_guess_localip(int family, char *address, int size  MARGS)
{
	return eXosip_guess_ip_for_via(family, address, size MPARAMS);
}

int eXosip_is_public_address(const char *c_address   MARGS)
{
...
void eXosip_set_user_agent(const char *user_agent MARGS)
{
...
void eXosip_quit( eXosip_t *_pXosip )
{
	...
	__eXosip_wakeup_event( MPARAMS2 );
	...
	eXosip_kill_transaction(&eXosip.j_osip->osip_ict_transactions  MPARAMS);
	eXosip_kill_transaction(&eXosip.j_osip->osip_nict_transactions MPARAMS);
	eXosip_kill_transaction(&eXosip.j_osip->osip_ist_transactions MPARAMS);
	eXosip_kill_transaction(&eXosip.j_osip->osip_nist_transactions MPARAMS);
	...
	eXtl_udp.tl_free( MPARAMS2);
	eXtl_tcp.tl_free( MPARAMS2);
#ifdef HAVE_OPENSSL_SSL_H
#if !(OPENSSL_VERSION_NUMBER < 0x00908000L)
	eXtl_dtls.tl_free( MPARAMS2);
#endif
	eXtl_tls.tl_free( MPARAMS2);
#endif

#ifndef SIP_INSTANCE_ONE 
	eXosip_xtl_uninit_udp( &(eXosip.xtl_udp) );	
	eXosip_xtl_uninit_tcp( &(eXosip.xtl_tcp));
#ifndef DISABLE_TLS
	eXosip_xtl_uninit_tls(&(eXosip.xtl_tls));
	eXosip_xtl_uninit_dtls(&(eXosip.xtl_dtls));
#endif
#endif
...
#ifdef OSIP_MT
	eXosip.j_thread = (void *) osip_thread_create(20000, _eXosip_thread, &eXosip );
	if (eXosip.j_thread == NULL) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
...
#ifndef MINISIZE
int eXosip_find_free_port(int free_port, int transport  MARGS)
{
	int res1;
	int res2;
	...
	for (count = 0; count < 8; count++) {
		if (ipv6_enable == 0)
			res1 = eXosip_get_addrinfo(&addrinfo_rtp, "0.0.0.0", free_port + count * 2, transport   MPARAMS);
		else
			res1 = eXosip_get_addrinfo(&addrinfo_rtp, "::", free_port + count * 2, transport MPARAMS);
		if (res1 != 0)
			return res1;
		if (ipv6_enable == 0)
			res2 = eXosip_get_addrinfo(&addrinfo_rtcp, "0.0.0.0", free_port + count * 2 + 1, transport MPARAMS);
		else
			res2 = eXosip_get_addrinfo(&addrinfo_rtcp, "::", free_port + count * 2 + 1, transport MPARAMS);
		if (res2 != 0) {
...
#ifdef IPV6_V6ONLY
				if (setsockopt_ipv6only(sock MPARAMS)) {
					close(sock);
					sock = -1;
					OSIP_TRACE(osip_trace
							   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	if (ipv6_enable == 0)
		res1 = eXosip_get_addrinfo(&addrinfo_rtp, "0.0.0.0", 0, transport MPARAMS);
	else
		res1 = eXosip_get_addrinfo(&addrinfo_rtp, "::", 0, transport MPARAMS);
 ...
 int eXosip_get_listen_port (MARGS2)
 {
	 if( eXosip.eXtl == NULL)
	 {
		 return -1;
	 }
	return eXosip.eXtl->proto_port;
 }
				   ...
				   int secure  MARGS)
{
	...
	{
		for( port  = 15060; port<(15060+3000); port++ )
		{
			eXtl->proto_port = port;
			i = eXtl->tl_open( MPARAMS2);
			if (i == 0)
...
int eXosip_init(  eXosip_t **ppEXosip )
{
	osip_t *osip;
	int i;
...
#ifndef SIP_INSTANCE_ONE 
	eXosip_t *_pXosip = NULL;
	*ppEXosip = NULL;
	_pXosip = (eXosip_t *)malloc(sizeof(eXosip_t));
	if( NULL == _pXosip )
	{
		return OSIP_NOMEM;
	}
	...
	*ppEXosip = NULL;
...
#ifndef SIP_INSTANCE_ONE 
	eXosip_xtl_init_default_udp( &(eXosip.xtl_udp) );	
	eXosip_xtl_init_default_tcp( &(eXosip.xtl_tcp));
#ifndef DISABLE_TLS
	eXosip_xtl_init_default_tls(&(eXosip.xtl_tls));
	eXosip_xtl_init_default_dtls(&(eXosip.xtl_dtls));
#endif
#endif
	...
	eXosip.user_agent = osip_strdup("GoSunCN Sip UA/1.0");
	...
	eXosip_set_callbacks(osip MPARAMS);
	...
	eXtl_udp.tl_init( MPARAMS2);
	eXtl_tcp.tl_init( MPARAMS2);
#ifdef HAVE_OPENSSL_SSL_H
#if !(OPENSSL_VERSION_NUMBER < 0x00908000L)
	eXtl_dtls.tl_init(MPARAMS2);
#endif
	eXtl_tls.tl_init(MPARAMS2);
#endif
	return OSIP_SUCCESS;
}


int eXosip_execute( MARGS2 )
{
	...
	lower_tv.tv_usec = 0;
#endif
	i = eXosip_read_message(1, lower_tv.tv_sec, lower_tv.tv_usec MPARAMS);
	...
	eXosip_release_terminated_calls( MPARAMS2);
	eXosip_release_terminated_registrations( MPARAMS2);
	eXosip_release_terminated_publications( MPARAMS2);
#ifndef MINISIZE
	eXosip_release_terminated_subscriptions( MPARAMS2);
	eXosip_release_terminated_in_subscriptions( MPARAMS2);
#endif

	eXosip_unlock( MPARAMS2);

	_eXosip_keep_alive( MPARAMS2);

	return OSIP_SUCCESS;
}

int eXosip_set_option(int opt, const void *value  MARGS)
{
	int val;
	char *tmp;
	...
	case EXOSIP_OPT_ADD_DNS_CACHE:
		{
			...
			if (entry->ip[0] == '\0') {
				...
				/* create the A record */
				i = eXosip_get_addrinfo(&addrinfo, entry->host, 0, IPPROTO_UDP MPARAMS);
				if (i!=0)
					return OSIP_BADPARAMETER;
...
static void _eXosip_keep_alive( MARGS2 )
{
	...
	eXtl_udp.tl_keepalive( MPARAMS2);
	eXtl_tcp.tl_keepalive( MPARAMS2);
#ifdef HAVE_OPENSSL_SSL_H
	eXtl_tls.tl_keepalive( MPARAMS2);
#if !(OPENSSL_VERSION_NUMBER < 0x00908000L)
	eXtl_dtls.tl_keepalive( MPARAMS2);
#endif
#endif
}
