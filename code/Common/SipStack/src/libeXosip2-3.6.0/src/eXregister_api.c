...
static int _eXosip_register_build_register(eXosip_reg_t * jr,
										   osip_message_t ** _reg MARGS);

static eXosip_reg_t *eXosip_reg_find(int rid MARGS)
{
	eXosip_reg_t *jr;
...
int eXosip_register_remove(int rid MARGS)
{
	eXosip_reg_t *jr;
	...
	jr = eXosip_reg_find(rid MPARAMS);
	if (jr == NULL)
...
static int
_eXosip_register_build_register(eXosip_reg_t * jr, osip_message_t ** _reg MARGS)
{
	...
	*_reg = NULL;
	...
	if (jr->r_last_tr != NULL) {
		if (jr->r_last_tr->state != NICT_TERMINATED
			...
			__eXosip_delete_jinfo(jr->r_last_tr MPARAMS);
			tr = jr->r_last_tr;
			jr->r_last_tr = NULL;
			...
			/* modify the REGISTER request */
			{
				...
				i = eXosip_update_top_via(reg MPARAMS);
				if (i != 0) {
					osip_message_free(reg);
			...
			if (last_response != NULL) {
				if (last_response->status_code == 401
					...
					eXosip_add_authentication_information(reg, last_response MPARAMS);
				} else
					eXosip_add_authentication_information(reg, NULL MPARAMS);
				osip_message_free(last_response);
			}
		}
	}
									   ...
									   osip_message_t ** reg MARGS)
{
	...
	*reg = NULL;
	...
	if (jr == NULL) {
		...
		i = eXosip_reg_init(&jr, from, proxy, contact MPARAMS);
		if (i != 0) {
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_ERROR, NULL,
	...
	i = _eXosip_register_build_register(jr, reg MPARAMS);
	if (i != 0) {
		OSIP_TRACE(osip_trace
				   (__FILE__, __LINE__, OSIP_ERROR, NULL,
...
eXosip_register_build_initial_register(const char *from, const char *proxy,
									   ...
									   osip_message_t ** reg MARGS)
{
	return eXosip_register_build_initial_register_withqvalue(from, proxy, contact, expires, NULL, reg MPARAMS);
}

int eXosip_register_build_register(int rid, int expires, osip_message_t ** reg MARGS)
{
	eXosip_reg_t *jr;
	int i;
...
int eXosip_register_send_register(int rid, osip_message_t * reg MARGS)
{
	...
	i = _eXosip_transaction_init(&transaction, NICT, eXosip.j_osip, reg MPARAMS);
	if (i != 0) {
		osip_message_free(reg);
		return i;
	}
