...
int
eXosip_remove_transaction_from_call(osip_transaction_t * tr, eXosip_call_t * jc MARGS)
{
												 ...
												 const char *method MARGS)
{
	...
	inc_tr = eXosip_find_last_inc_transaction(jc, jd, method MPARAMS);
	out_tr = eXosip_find_last_out_transaction(jc, jd, method MPARAMS);
	if (inc_tr == NULL)
		return out_tr;
	if (out_tr == NULL)
		return inc_tr;
											...
											eXosip_dialog_t * jd MARGS)
{
	...
	inc_tr = eXosip_find_last_inc_invite(jc, jd MPARAMS);
	out_tr = eXosip_find_last_out_invite(jc, jd MPARAMS);
	if (inc_tr == NULL)
		return out_tr;
	if (out_tr == NULL)
		return inc_tr;
...
osip_transaction_t *eXosip_find_previous_invite(eXosip_call_t * jc,
												eXosip_dialog_t * jd,
												osip_transaction_t * last_invite MARGS)
{
	osip_transaction_t *inc_tr;
	osip_transaction_t *out_tr;
	int pos;
