...
static const char *s_csNotice = "NOTICE";
...
{
	if(!g_bModuleRunning || (!( eLevel&m_iLVDirConsole ) && !( eLevel&m_iLVDirFile ) ) )
	{
		return;
	}
CGSPString strTemp;
...
const char *czTempPrefix = czPrefix;
	...
	else  if( eLevel&m_iLVDirConsole )
	{
		MY_DEBUG_PRINTF("%s %s %s", GetLevelName(eLevel), czPrefix,  strTemp.c_str() );
	}
}
...
CFunctionDebug::CFunctionDebug(CLog *pLog, const CGSString &strFunc, const char  *czFormat,...)
{
	...
	{
		MY_DEBUG_PRINTF("DEBUG FUNC >>>%lld<<< **%s** Enter %s.\n",
			(long long) DoGetTickCount(), 
			m_strFunc.c_str(), 
			m_strInfo.c_str());
	}
