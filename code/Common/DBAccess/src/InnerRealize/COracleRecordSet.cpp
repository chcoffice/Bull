...
// 游标索引
INT		COracleRecordSet::m_iCursorIndex = 0;
COracleRecordSet::COracleRecordSet(void)
{
#if 0
	// 创建数据集实例
	m_pRecordset = NULL;
...
/**************************************************************************************************
  ...
  Note:				
**************************************************************************************************/
BOOL	COracleRecordSet::MoveLast()
{
#if 0
	//	ASSERT(m_pRecordset != NULL);
...
Note			:
**************************************************************************************************/
BOOL	COracleRecordSet::AddNew(const	char*	szTableName)
{
	// 初始化参数
	Init();
...
**************************************************************************************************/
BOOL	COracleRecordSet::GetCollect(const char* szFieldName,string&	szValue)
{
#if 0
	//	ASSERT(m_pRecordset != NULL);
	// 获取指定字段的值
...
INT COracleRecordSet::GetColumnNumber(void)
{
	if(m_hSqlStmt == SQL_NULL_HSTMT)
	{
		return	-1;
...
BOOL COracleRecordSet::GetCloumnName(INT iColIndex, std::string &oStrName )
{
	oStrName.clear();
...
**************************************************************************************************/
BOOL	COracleRecordSet::PutCollect(const char* szFieldName,const char*     szValue)
{
#if 0
	//	ASSERT(m_pRecordset != NULL);
	try
...
BOOL COracleRecordSet::GetBlobCollect(const char* szFieldName,string&	strValue)
{
	try
	{	
		// 当前字段的字节长度
		...
		// 获取数据大小
		iRetFlag = SQLGetData(m_hSqlStmt,iColumnIndex,SQL_C_BINARY,buf,0,&iActualDataSize);
		if (SQL_SUCCEEDED(iRetFlag))
		{
			if ( iActualDataSize>0 )
			{
				...
				iRetFlag = SQLGetData(m_hSqlStmt,iColumnIndex,SQL_C_BINARY,&(strValue[0]),strValue.size(),&iActualDataSize);
...
/**************************************************************************************************
  ...
  Note			:
**************************************************************************************************/
BOOL	COracleRecordSet::PutDtCollect(const char* szFieldName,const INT iYear,const INT iMonth,const INT iDay,const INT iHour,const INT iMinute,const INT iSecond)
{
#if 0
...
**************************************************************************************************/
BOOL	COracleRecordSet::ReleaseRecordSet()
{
	delete	this;
	return  TRUE;
}
...
**************************************************************************************************/
BOOL	COracleRecordSet::GetCollect(const char* szFieldName,INT&	iValue)
{
#if 0
	//	ASSERT(m_pRecordset != NULL);
	try
...
**************************************************************************************************/
BOOL	COracleRecordSet::PutCollect(const char* szFieldName,const INT	iValue)
{
#if 0
	//	ASSERT(m_pRecordset != NULL);
	try
...
/**************************************************************************************************
  ...
  Note:				用于操作二进制大对象
**************************************************************************************************/
BOOL	COracleRecordSet::GetCollect(const char* szFieldName,void*	   pValue,const INT iBuffSize,INT&	iDataLen)
{
#if 0
...
/**************************************************************************************************
  ...
  Note:				用于操作二进制大对象
**************************************************************************************************/
BOOL	COracleRecordSet::PutCollect(const char* szFieldName,const  void*	   pValue,const INT	iDataLen)
{
#if 0
	// 获取字段集指针
	FieldPtr		pField = GetField(szFieldName);
...
/**************************************************************************************************
  ...
  Note:				执行SELECT查询操作
**************************************************************************************************/
IRecordSet*		COracleRecordSet::QuerySql(const char*	szSql)
{	
	//	ASSERT(m_pRecordset != NULL)
	// 执行操作返回数据集对象
	if (Open(szSql))
...
/**************************************************************************************************
  ...
  Note:				NULL
**************************************************************************************************/
BOOL		COracleRecordSet::GetEditMode()
{
#if 0
	//	ASSERT(m_pRecordset != NULL);
...
Note:				// 备注
**************************************************************************************************/
BOOL			COracleRecordSet::SetConnection(COracleConnection* pConnection)
{
	if (pConnection != NULL)
	{
...
/**************************************************************************************************
  ...
  Note:				
**************************************************************************************************/
BOOL	COracleRecordSet::CancelUpdate()
{
#if 0
	//	ASSERT(m_pRecordset != NULL);
...
/**************************************************************************************************
  ...
  Note:			
**************************************************************************************************/
BOOL	COracleRecordSet::Open(const char*  szSql)
{
#if 0
	// 连接对象和数据集对象的有效性检查
	...
	try
	{
		...
		{	
			...
			{
				INT iErrno = 0;
				ErrorLogInfo(m_pConnection->GetOdbcSqlHEven(),m_pConnection->GetOdbcSqlHDbc(),m_hSqlStmt,&iErrno);
				//----------------------------------------------------------------------------------
				//Close(); 
...
/**************************************************************************************************
  ...
  Note:				
**************************************************************************************************/
FieldsPtr COracleRecordSet::GetFields()
{
//	ASSERT(m_pRecordset != NULL);
	try
	{
...
/**************************************************************************************************
  ...
  Note:		
**************************************************************************************************/
FieldPtr  COracleRecordSet::GetField(const char* szFieldName)
{
	try
	{
...
BOOL		COracleRecordSet::Init()
{
	...
	{ 
		// 先判断句柄是否可用
		if ( m_pConnection->GetOdbcSqlHDbc() == SQL_NULL_HANDLE )
		{
			return FALSE;
		}

		if ( !m_pConnection->IsOpen() )   // modified by jiangsx 20120914  修正多连接操作时，断线重连问题
...
/**************************************************************************************************
  ...
  Note			: NULL
**************************************************************************************************/
BOOL		COracleRecordSet::GetColumnList()
{
	if (m_hSqlStmt == SQL_NULL_HSTMT)
	{
		return FALSE;
...
/**************************************************************************************************
  ...
  Note			: NULL
**************************************************************************************************/
BOOL		COracleRecordSet::ClearColumnList()
{
	m_ColumnItemList.clear();
	return	TRUE;
...
**************************************************************************************************/
INT		  COracleRecordSet::GetColumnIndexByName(const char* szFieldName)
{
	if(m_hSqlStmt == SQL_NULL_HSTMT)
	{
...
**************************************************************************************************/
BOOL		COracleRecordSet::ClearFieldInfoList()
{
	// 清空字段列表，
...
// 向字段列表中添加字段信息
BOOL		COracleRecordSet::AddFieldItem2FieldInfoList(CDBFieldInfo*	pcsDbFieldInfoItem)
{	
	// 先判断列名是否存在
	for (DBFieldInfoVector::size_type i = 0 ;i <m_FieldInfoList.size();i++)
	{
...
**************************************************************************************************/
BOOL		COracleRecordSet::GetTableNameFromSqlText(const char* szSql,string& strTableName)
{
	try
	{
...
Note			: 
**************************************************************************************************/
BOOL		COracleRecordSet::BuildInsertUpdateSqlTxt()
{
	// 确保有元素存在
...
**************************************************************************************************/
BOOL		COracleRecordSet::AppendValueToSqlTxt()
{
	// 确保有元素存在
...
**************************************************************************************************/
BOOL		COracleRecordSet::ExecuteInsertUpdate()
{
	// 执行操作
	if (m_hUpdateSqlStmt != SQL_NULL_HSTMT)
	{
